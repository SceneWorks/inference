...
//! branch stays bf16. Render B wraps `mlx_gen::load` in a lowered MLX memory limit (`KREA_BQ_STARVE_GIB`,
...
// Force-link the provider crate so its `inventory` generator registration for `krea_2_turbo_control`
// survives linker GC and `mlx_gen::load` can resolve it.
...
fn render(size: u32, starve: bool) -> (Image, f64) {
    let model = if starve {
        ...
        let m = mlx_gen::load("krea_2_turbo_control", &spec());
        set_memory_limit(prev); // restore so the render has memory
        m
    } else {
        mlx_gen::load("krea_2_turbo_control", &spec())
    }
