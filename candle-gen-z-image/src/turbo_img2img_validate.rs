...
//! (`gen_core::registry::load("z_image_turbo", …)`) through a [`Conditioning::Reference`] against the
...
fn real_weight_turbo_img2img() {
    ...
    let model = gen_core::registry::load("z_image_turbo", &spec).expect("load z_image_turbo");
    println!(
