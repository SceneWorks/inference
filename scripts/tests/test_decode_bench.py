...
class DecodeBenchWrapperTests(unittest.TestCase):
    ...
    def test_run_seals_suite_document_and_renders_table(self) -> None:
        ...
        self.assertIn(
            "| head-test | MTP off (reference) | 4 | (ref) | yes | 10.00 | n/a | 1.000 | n/a | 1.00 GiB | n/a | n/a |",
            table,
        )
        self.assertIn(
            "| head-test | MTP off (StepModel) | 4 | yes | yes | 10.50 | n/a | 1.000 | 1.00 | 1.00 GiB | 3.0 MiB | 2.0 MiB |",
            table,
        )
        self.assertIn(
            "| head-test | MTP K=3 | 4 | no @2 | no @2 | 15.50 | 0.500 | 0.750 | 4.00 | 1.50 GiB | n/a | n/a |",
            table,
        )
    ...
    def test_step_model_row_label_names_the_kv_cache_when_reported(self) -> None:
        ...
        self.assertEqual(
            bench.row_label({"path": "step_model", "kv_cache": "growing"}),
            "MTP off (StepModel, growing kv)",
        )
        self.assertEqual(bench.row_label({"path": "reference", "kv_cache": "growing"}), "MTP off (reference)")
