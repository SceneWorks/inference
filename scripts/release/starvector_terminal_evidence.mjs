...
export const LIMIT_BEHAVIOR_PROFILE_SHA256 = "829c941e076aefeccbdc3c79eaa7ae05889447766dd2defadfa392ca2eb08320";
const OUTCOME_PARITY_PROFILE_ID = "https://sceneworks.dev/schemas/starvector-terminal-receipt-v2-outcome-parity.json";
const LIMIT_BEHAVIOR_PROFILE_ID = "https://sceneworks.dev/schemas/starvector-terminal-receipt-v2-outcome-parity-limit-behaviors.json";
...
const LIMIT_BEHAVIORS = ["completion", "token", "byte", "wall_time", "queued_cancellation", "in_flight_cancellation"];
...
function run(value, refs, version = 1, limitKeys = LIMITS) {
...
function inferLimitKeys(receipt) {
  if (!Array.isArray(receipt?.runs) || receipt.runs.length === 0) fail("run count");
  const signatures = new Set(receipt.runs.map((entry) => Object.keys(entry?.limits ?? {}).sort().join("|")));
  if (signatures.size !== 1) fail("mixed limit behavior contracts");
  const signature = [...signatures][0];
  if (signature === [...LIMITS].sort().join("|")) return LIMITS;
  if (signature === [...LIMIT_BEHAVIORS].sort().join("|")) return LIMIT_BEHAVIORS;
  fail("unknown limit behavior contract");
}
function currentEvidenceRefs(receipt, corpus, limitKeys = inferLimitKeys(receipt), expectedPreflightRevision = receipt.inference_revision) { const refs = []; metric(receipt.metric_identity, refs); preflight(receipt.inference_preflight, expectedPreflightRevision, refs); ref(refs, "producer/transcript", receipt.producer.transcript_sha256); receipt.runs.forEach((entry) => run(entry, refs, receipt.schema_version, limitKeys)); hostile(receipt.hostile_sanitizer, corpus, refs); prompt(receipt.prompt_composition, corpus, refs); return refs; }
...
export function currentArtifactReferences(receipt, corpus, expectedPreflightRevision = receipt.inference_revision) {
  return currentEvidenceRefs(receipt, corpus, inferLimitKeys(receipt), expectedPreflightRevision).map(({ path, sha256 }) => ({ path, sha256 }));
}
export function buildArtifactManifest(receipt, corpus, currentArtifactByteSizes, expectedPreflightRevision = receipt.inference_revision) {
  const refs = currentArtifactReferences(receipt, corpus, expectedPreflightRevision);
...
export function validateReceipt(receipt, corpusHash, inferenceRevision, sceneworksRevision, corpus, currentArtifactByteSizes, profileSha256 = OUTCOME_PARITY_PROFILE_SHA256, expectedPreflightRevision = inferenceRevision) {
  ...
  validateReceiptBody(legacy, corpusHash, inferenceRevision, sceneworksRevision, corpus, 2, limitKeys, expectedPreflightRevision);
  ...
  const expected = buildArtifactManifest(receipt, corpus, currentArtifactByteSizes, expectedPreflightRevision);
...
export function validateReceiptProfile(receipt, profilePath, expectedSha256) {
  const selected = expectedSha256 === OUTCOME_PARITY_PROFILE_SHA256
    ? { id: OUTCOME_PARITY_PROFILE_ID, limits: LIMITS }
    : expectedSha256 === LIMIT_BEHAVIOR_PROFILE_SHA256
      ? { id: LIMIT_BEHAVIOR_PROFILE_ID, limits: LIMIT_BEHAVIORS }
      : fail("selected receipt profile hash is not a supported outcome-parity contract");
  ...
  const parityVersion = profile?.$defs?.upstream_parity?.properties?.contract_version?.const
    ?? (profile?.$defs?.run?.properties?.deterministic_parity?.$ref === `${OUTCOME_PARITY_PROFILE_ID}#/$defs/upstream_parity` ? 2 : undefined);
  if (profile?.$id !== selected.id || profile?.properties?.schema_version?.const !== 2 || parityVersion !== 2) fail("selected receipt profile identity is not outcome parity V2");
  if (expectedSha256 === LIMIT_BEHAVIOR_PROFILE_SHA256) {
    const definition = profile?.$defs?.limit_behaviors;
    if (!definition || definition.additionalProperties !== false || [...(definition.required ?? [])].sort().join("|") !== [...LIMIT_BEHAVIORS].sort().join("|") || LIMIT_BEHAVIORS.some((name) => definition.properties?.[name]?.const !== true)) fail("selected receipt profile does not bind executable limit behaviors");
  }
  ...
  receipt.runs.forEach((entry) => { keys(entry.limits, selected.limits, "selected profile limits"); if (Object.values(entry.limits).some((value) => value !== true)) fail("selected profile limits incomplete"); });
...
function main() {
  ...
  if (command === "validate-receipt") {
    ...
    const inferenceRevision = option("--inference-revision");
    const expectedPreflightRevision = optional("--preflight-inference-revision") ?? inferenceRevision;
    const evidenceRoot = receipt.schema_version === 2 ? option("--evidence-root") : undefined;
    const sizes = evidenceRoot === undefined ? undefined : artifactByteSizesFromFiles(evidenceRoot, currentArtifactReferences(receipt, corpus, expectedPreflightRevision));
    validateReceipt(receipt, validatePlan(corpus), inferenceRevision, option("--sceneworks-revision"), corpus, sizes, profileSha256, expectedPreflightRevision);
