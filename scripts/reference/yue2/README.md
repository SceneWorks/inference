...
| `nar_fixtures.py` | `synthetic`: the committed CI fixture for the native acoustic stage (`crates/audio/candle-audio-yue2/tests/fixtures/nar_synthetic.*`) — upstream `yue2.nar.synthesize` / `CachedNAR` / `nar_velocity` on the synthetic MoT with hashed NAR heads, from upstream's own full-song noise, every velocity evaluation recorded. `real`: YuE2-3B in F32; the latents derive from CC BY-NC weights so they are written outside the repository (`~/.cache/sceneworks-yue2-fixtures/nar`) and only their SHA-256 and the cases are committed (`tests/fixtures/nar_real_reference.json`). Peak RSS: see that directory's README. |
| `fp8_fixtures.py` | `crates/audio/candle-audio-yue2/tests/fixtures/fp8_quantize.json` — upstream `yue2.quantization.quantize_tensor` (the experimental FP8 AR mode's per-tensor E4M3 quantizer) on four generated BF16 tensors: the exact F32 scale and E4M3 codes the native `fp8::quantize_e4m3` must reproduce (sc-22995). No weights; peak RSS ~0.4 GB. |
| `vae_reference.py` | `tiny`: the committed CI fixture for the native VAE (`crates/audio/candle-audio-yue2/tests/fixtures/vae_tiny*`) — two released-topology VAEs at toy widths with randomized weights (standard/legacy), upstream `decode`, `YuE2Pipeline.decode` output and encoder posterior for one shared latent. `real`: the pinned-weight reference (both decoders on the same encoded latents); the waveforms are derived from CC BY-NC weights so they are written outside the repository (`~/.cache/sceneworks-yue2-fixtures/vae`) and only their SHA-256 and statistics are committed (`tests/fixtures/vae_real_reference.json`). Peak RSS measured at 0.34 GB (tiny) / 3.0 GB (real). |
