    ...
    * a *call* to ``validate`` on a route-named receiver.
    ...
    for member_id in metadata["workspace_members"]:
        ...
        if "register_memory_strategy" not in joined:
            continue
        rung_two_markers = [marker for marker in PID_RUNG_TWO_MARKERS if marker in joined]
        ...
        if rung_two_markers == ["configure_decode"] and _configure_decode_hooks_are_unconditional_rejections(joined):
            continue
        missing: list[str] = []
