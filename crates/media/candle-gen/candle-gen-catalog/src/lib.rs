...
mod tests {
    ...
    /// Epic 20414 / sc-20416 gating. The two advanced multi-pass schedules are implemented in shared
    /// gen-core for every backend, but they are advertised ONLY by the model families explicitly
    /// validated for them. Krea 2 is the epic's first acceptance target and is currently the whole
    /// allow-list; every other registered family must still show exactly the menu it showed before.
    /// Widening this set is a deliberate edit here plus the family's own validation — never a side
    /// effect of touching the shared vocabulary.
    #[test]
    fn advanced_pass_schedulers_are_gated_to_validated_families_only() {
        const VALIDATED_FAMILIES: [&str; 1] = ["krea_2"];
        let advanced = candle_gen::advanced_pass_scheduler_names();
        ...
        for registration in registry.generators() {
            ...
            if validated {
                ...
                for curated in candle_gen::curated_scheduler_names() {
                    assert!(
