//! ViT (Qwen2.5-VL) image/video preprocessing — packed patch pixels + `grid_thw` for the
//! [`crate::vision::VisionTower`]. Candle sibling of `mlx-gen-bernini/src/vit_preprocess.rs` (sc-5136).
//!
//! Port of `Qwen2VLImageProcessor`:
...
//! resampled pixels differ slightly. The exactly-matchable pieces (`smart_resize` dims, the
//! rescale+normalize+pack on a fixed input, and `smart_video_nframes`) are goldened bit-for-bit.
...
pub fn pack_patches(
    ...
    let (f, c, h, w) = frames.dims4()?;
    let (f, c, h, w) = (f as i64, c as i64, h as i64, w as i64);

    // Temporal-pad to a multiple of `temporal` by repeating the last frame.
    let frames = if f % temporal != 0 {
        let pad = temporal - (f % temporal);
        let idx: Vec<u32> = vec![(f - 1) as u32; pad as usize];
        let sel = Tensor::from_vec(idx, (pad as usize,), frames.device())?;
        let last = frames.index_select(&sel, 0)?;
        Tensor::cat(&[frames, &last], 0)?
    } else {
        frames.clone()
    };

    let fp = frames.dim(0)? as i64;
    let grid_t = fp / temporal;
    let grid_h = h / patch;
    let grid_w = w / patch;
    let (gh, gw) = (grid_h / merge, grid_w / merge);
    let u = |x: i64| x as usize;
    ...
    let perm = reshaped
        .permute([0usize, 3, 6, 4, 7, 2, 1, 5, 8].as_slice())?
        .contiguous()?;
    let seq = grid_t * grid_h * grid_w;
    let row = c * temporal * patch * patch;
    let pixel_values = perm.reshape(&[u(seq), u(row)])?;
    Ok((pixel_values, [grid_t as i32, grid_h as i32, grid_w as i32]))
}
...
/// conditioning — the latter passes the CLIP [`IMAGE_MEAN`] / [`IMAGE_STD`].
pub(crate) fn normalized_frame(
    ...
    for (c, (&m, &sd)) in mean.iter().zip(std.iter()).enumerate() {
        for y in 0..hu {
            for x in 0..wu {
                let px = pixels_hwc[(y * wu + x) * 3 + c] as f32;
                data[(c * hu + y) * wu + x] = (px / 255.0 - m) / sd;
            }
        }
    }
    Ok(Tensor::from_vec(data, (1, 3, hu, wu), dev)?)
}
...
pub fn preprocess_image(
    ...
    let frame = normalized_frame(resized.as_raw(), rh, rw, mean, std, dev)?;
...
mod tests {
    ...
    fn pack_shapes() {
        ...
        assert_eq!(pv.dims(), &[24, 1176]); // seq = 1*4*6, row = 3*2*14*14
    }
}
