//! Beta-distribution sigma schedule for `chroma1_base`.
//!
...
//!
//! Pure f64 host math (no tensor backend), so this is a verbatim carry-over from `mlx-gen-chroma`'s
//! `beta.rs` — the "carries over via gen-core" reuse the epic describes.
