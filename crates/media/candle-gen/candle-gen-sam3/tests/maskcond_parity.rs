//! SAM3 mask-conditioned (detection-seeded) frame decode parity (sc-6245): feed the captured raw
//! image embedding + high-res features + detection mask into the candle
//! `Sam3Tracker::decode_mask_conditioning_frame` and check the high-res mask logits + object pointer +
//! object score against the SAME torch oracle the MLX twin uses
//! (`mlx-gen/scripts/spikes/sam3_oracle/dump_maskcond_fixture.py`). `#[ignore]` until weights +
//! fixture are staged (sc-6248). Run:
//!   SAM3_WEIGHTS=<snapshot> SAM3_MASKCOND_FIXTURE=<maskcond_fixture.safetensors> \
//!     cargo test -p candle-gen-sam3 --release --features cuda --test integration maskcond_parity:: -- --ignored --nocapture
...
#[test]
#[ignore = "needs staged facebook/sam3 weights (SAM3_WEIGHTS) + SAM3_MASKCOND_FIXTURE — sc-6248"]
fn mask_conditioning_decode_matches_oracle() {
    ...
    let fixture_path = std::env::var("SAM3_MASKCOND_FIXTURE")
        .expect("set SAM3_MASKCOND_FIXTURE to the oracle dump");

    let device = default_device().expect("default device");
    let w = load_weights(&weights_path, &device);
    let fx = Weights::from_file(&fixture_path, &device).expect("load maskcond fixture");
