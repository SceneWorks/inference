...
/// The **gated** advanced multi-pass scheduler menu (epic 20414, sc-20416): `linear_quadratic` and
/// `bong_tangent`. Deliberately NOT part of [`curated_scheduler_names`] — a family advertises these
/// only once it has been explicitly validated for them, by appending this to its `schedulers` list
/// (`menu_with_aliases(curated_scheduler_names(), &advanced_pass_scheduler_names())`). Selecting one
/// builds the schedule through the same [`resolve_schedule`] seam, so MLX and candle consume
/// identical coefficients (`gen_core_testkit::schedules`).
pub fn advanced_pass_scheduler_names() -> Vec<&'static str> {
    gen_core::sampling::Scheduler::ADVANCED_PASS
        .iter()
        .map(|s| s.name())
        .collect()
}
...
mod tests {
    ...
    fn candle_drives_every_curated_solver_to_finite_output() {
        ...
        // Iterates `Solver::ALL` (like the MLX twin, since PR #704) rather than a hand-maintained
        // name list, so a solver added to the registry can never be silently missing here.
        let ops = CandleLatentOps;
        let ms = FlowModelSampling::new(TimestepConvention::Sigma);
    ...
    /// The curated menus expose exactly the gen-core vocabulary (decision 2) — and the epic-20414
    /// advanced schedules are gated OUT of it (sc-20416): a family opts in explicitly via
    /// [`super::advanced_pass_scheduler_names`], so every family that did not is byte-identical.
    #[test]
    fn curated_menus_match_vocabulary() {
        ...
        assert_eq!(
            ...
            vec!["linear_quadratic", "bong_tangent"]
        );
        for gated in advanced_pass_scheduler_names() {
            assert!(
                !curated_scheduler_names().contains(&gated),
                "{gated} must stay out of the ungated curated menu"
            );
        }
        // The opt-in menu is the concatenation, with no duplicates (the registry's
        // `check_name_list` conformance sweep rejects a repeated entry).
        let opted_in =
            super::menu_with_aliases(curated_scheduler_names(), &advanced_pass_scheduler_names());
        assert_eq!(opted_in.len(), 10);
        let mut sorted = opted_in.clone();
        sorted.sort_unstable();
        sorted.dedup();
        assert_eq!(sorted.len(), 10, "opt-in menu must have no duplicates");
    }

    /// The candle half of the epic-20414 shared-schedule seam (sc-20416).
    ///
    /// `gen_core_testkit::schedules` owns ONE checked-in table of independently derived σ vectors
    /// (regenerated by `gen-core-testkit/fixtures/gen_schedule_golden.py`). This replays it through
    /// `candle_gen::resolve_schedule`; the mlx-gen twin
    /// (`mlx_gen::sampler::tests::mlx_seam_delivers_the_golden_schedule_coefficients`) replays the
    /// SAME table through `mlx_gen::resolve_schedule`. Both passing is the statement that the two
    /// backends receive identical coefficients.
    #[test]
    fn candle_seam_delivers_the_golden_schedule_coefficients() {
        gen_core_testkit::schedules::schedule_conformance("candle", &|name, ms, steps| {
            super::resolve_schedule(Some(name), ms, steps, &[])
        });
    }

    /// The seam is a pure delegation: for every scheduler id (curated AND gated) at every step
    /// count it returns exactly what the shared host math returns — no rounding, no re-striding, no
    /// backend-local branch. The N1/N3 fallbacks are unchanged by the additions, including the
    /// epic's documented `bong_tangente` misspelling, which must fall back rather than resolve.
    #[test]
    fn candle_seam_is_a_pure_delegation_and_keeps_its_native_fallbacks() {
        use gen_core::sampling::{schedule_sigmas, FlowModelSampling, ModelSampling};
        let flow = FlowModelSampling::new(TimestepConvention::Sigma);
        let ms: &dyn ModelSampling = &flow;
        let native = vec![1.0_f32, 0.7, 0.3, 0.0];
        for sched in Scheduler::EVERY {
            for steps in [1_usize, 2, 4, 10, 20, 50] {
                assert_eq!(
                    super::resolve_schedule(Some(sched.name()), ms, steps, &native),
        ...
        assert_eq!(super::resolve_schedule(None, ms, 3, &native), native);
        assert_eq!(
            super::resolve_schedule(Some("bong_tangente"), ms, 3, &native),
            native,
            "the epic's documented misspelling must fall back, not resolve"
        );
        assert_eq!(
            super::resolve_schedule(Some("flow_match"), ms, 3, &native),
            native
        );
    }

    /// Both advanced schedules resolve through the SAME `resolve_schedule` seam as the curated
    /// eight — no backend-local math — and produce a valid descending-to-0 schedule at the
    /// epic-20414 recipe lengths.
    #[test]
    fn advanced_pass_schedulers_resolve_through_the_shared_seam() {
        use super::{advanced_pass_scheduler_names, resolve_flow_schedule};
        use gen_core::sampling::{schedule_sigmas, FlowModelSampling, Scheduler};
        for steps in [4_usize, 10] {
            let mu = compute_mu(image_seq_len(1024, 1024), steps);
            let native = build_flow_sigmas(steps, mu);
            let ms = FlowModelSampling::with_shift(TimestepConvention::Sigma, mu);
            for name in advanced_pass_scheduler_names() {
                let got = resolve_flow_schedule(Some(name), mu, steps, &native);
                let want = schedule_sigmas(Scheduler::from_name(name).unwrap(), &ms, steps);
                assert_eq!(got, want, "{name} @ {steps} must be pure gen-core math");
                assert_eq!(got.len(), steps + 1, "{name}");
                assert_eq!(*got.last().unwrap(), 0.0, "{name}");
                assert!(got.windows(2).all(|w| w[0] > w[1]), "{name}: {got:?}");
                assert_ne!(got, native, "{name} must reshape the native schedule");
            }
        }
    }
