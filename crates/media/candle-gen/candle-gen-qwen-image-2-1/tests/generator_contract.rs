...
//! composition the SceneWorks worker reaches via `candle-gen-catalog`): validate honesty, progress
...
//! Runs by default on CPU: the tiny snapshot is ~500 KB and a render takes well under a second.
...
fn count_and_seed_reach_the_runtime() {
    ...
    assert_eq!(
        images[1].pixels, seeded[0].pixels,
...
/// The one deliberate difference from the MLX twin's contract suite: candle has no
/// affine-quantize-at-load path for this family, so `LoadSpec::quantize` is refused with a typed,
/// actionable `Unsupported` instead of quantizing the DiT.
#[test]
fn load_time_quantization_is_refused_with_a_typed_error() {
    ...
    let err = match registry.load(ID, &spec(OffloadPolicy::Resident).with_quant(Quant::Q8)) {
        Ok(_) => panic!("candle must refuse on-the-fly quantization"),
        Err(err) => err,
    };
