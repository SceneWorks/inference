//! **[`crate::layout`] Rule 2 made executable: the DiT's fused-QKV transform is NOT the video
//! VAE's.**
//!
...
//!   `crates/media/mlx-gen/tools/dump_minimax_h3_dit.py` — i.e. it runs at regeneration time,
//!   **not** in CI.
...
//! applies **no** transform at all. `tests/dit_parity.rs`'s
//! `the_published_checkpoint_ships_no_fused_qkv` re-verifies that against the fixture's own key set
//! rather than inheriting the MLX lane's finding. That is the whole reason this module exists as a
//! *contract* rather than as loader code: the transform is invisible in the port, so only an
//! explicit assertion can pin which one produced the bytes.
//!
...
pub fn split_thirds(fused: &Tensor, heads: usize, head_dim: usize) -> Result<[Tensor; 3]> {
    let s = fused.dims();
    ...
    for j in 0..3 {
        out.push(fused.narrow(0, j * inner, inner)?.contiguous()?);
    }
...
pub fn reorder_interleaved(fused: &Tensor, heads: usize, head_dim: usize) -> Result<Tensor> {
    let s = fused.dims();
    ...
    for j in 0..3 {
        parts.push(view.narrow(1, j, 1)?.contiguous()?.reshape(flat.clone())?);
    }
...
mod tests {
    ...
    fn raw(heads: usize, head_dim: usize) -> Tensor {
        let mut v = Vec::new();
        for h in 0..heads {
            for j in 0..3 {
                for d in 0..head_dim {
        ...
        Tensor::from_vec(v, (3 * heads * head_dim, 1), &Device::Cpu).unwrap()
    }
    ...
    fn split_thirds_takes_contiguous_thirds() {
        let (heads, head_dim) = (3usize, 2usize);
        ...
        assert_eq!(flat(&q), vec![0.0, 1.0, 100.0, 101.0, 200.0, 201.0]);
        assert_eq!(flat(&k), vec![10.0, 11.0, 110.0, 111.0, 210.0, 211.0]);
        assert_eq!(flat(&v), vec![20.0, 21.0, 120.0, 121.0, 220.0, 221.0]);
    }
    ...
    fn the_two_transforms_are_different_partitions() {
        let (heads, head_dim) = (4usize, 3usize);
    ...
    fn a_wrong_leading_dim_is_rejected() {
        let t = Tensor::from_vec(vec![1.0f32, 2.0, 3.0, 4.0, 5.0], (5, 1), &Device::Cpu).unwrap();
