...
//!
//! The candle twin of `mlx_gen_minimax_h3::dit::config`. Dimensions are `usize` here rather than
//! `i32`, because candle shapes are `usize`; every published value and every validation rule is the
//! same, and `tests/dit_parity.rs` holds both lanes to the same fixture.
...
pub const MODULATION_PARAMS: usize = 6;
...
impl MiniMaxH3DitConfig {
    ...
    pub fn adaln_out_features(&self) -> usize {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.rope_freq_dim == 0 {
            return Err(CandleError::Msg(
                "minimax-h3 dit: rope_freq_dim must be positive, got 0".into(),
            ));
        }
    ...
    /// variant checkpoint — a future H3 revision, or `transformer_ref` (sc-17157) — load and run
    ...
    pub fn from_diffusers_json(text: &str) -> Result<Self> {
        let v: serde_json::Value = serde_json::from_str(text)
            .map_err(|e| CandleError::Msg(format!("minimax-h3 dit config: {e}")))?;
        let missing = |k: &str| {
        ...
        let patch_size = v
            ...
            .and_then(|a| {
                ...
                for (slot, item) in out.iter_mut().zip(a) {
                    *slot = item.as_u64()? as usize;
                }
                Some(out)
            })
            .ok_or_else(|| {
                CandleError::Msg(
                    "minimax-h3 dit config: `patch_size` must be a 3-element array".into(),
                )
            })?;
...
mod tests {
    ...
    fn rejects_a_non_positive_rope_theta() {
        for theta in [0.0f64, -1.0, f64::NAN] {
