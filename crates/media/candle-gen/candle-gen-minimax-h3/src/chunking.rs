...
//! `mlx-gen-minimax-h3`'s so the two backends cannot disagree about the frame plan.
//!
...
impl TemporalGeometry {
    ...
    pub fn new(cfg: &MiniMaxH3VaeConfig) -> Result<Self> {
        let ratio_t = i32::try_from(cfg.patch_size_t).map_err(|_| {
            CandleError::Msg(format!(
                "minimax-h3 temporal geometry: patch_size_t {} does not fit in i32",
                cfg.patch_size_t
            ))
        })?;
        Self::from_parts(cfg.clip_length, cfg.token_drop, ratio_t)
    }
    ...
    pub fn from_parts(clip_length: i32, token_drop: i32, vae_ratio_t: i32) -> Result<Self> {
        ...
        // `ceil(clip_length / vae_ratio_t)`. Both are positive by the guard above, so the round
        // trip through `usize` is exact — and `usize::div_ceil` is stable where `i32`'s is not
        // (`int_roundings`), which is why this is not simply `clip_length.div_ceil(vae_ratio_t)`.
        let tokens_chunk_size = (clip_length as usize).div_ceil(vae_ratio_t as usize) as i32;
    ...
    /// are not modelled: the shipped checkpoint leaves all three `False`. Kept as a named constant
    /// so the boundary is greppable rather than implicit.
