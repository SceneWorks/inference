...
use crate::dit::positions::{
    ...
    temporal_grid, text_position_ids, to_tensor, video_position_ids, KeyframeAnchor,
...
impl PackedLayout {
    ...
    /// [`RowClass::ConditionAudio`] class be right on one path and silently absent on the other.
    /// `t2va` / `fl2va` pass `num_condition_audio_rows = 0` and take the same code.
    ...
    pub fn build_ref2va(
        ...
        let (target_frame_grid, target_width_grid) = frame_grid(
            geometry.latent_height,
            geometry.latent_width,
            patch_h,
            patch_w,
        ...
        let mut video_indices: Vec<u32> = Vec::new();
        let mut audio_indices: Vec<u32> = Vec::new();
        ...
        for (i, r) in references.iter().enumerate() {
            ...
            .map_err(|e| CandleError::Msg(format!("{e} (reference {i})")))?;
            // Audio FIRST — a clip's soundtrack rows are packed immediately before its video rows.
            for row in audio_rows {
                audio_indices.push(cursor as u32);
                rows.push(row);
                cursor += 1;
            }
            for row in video_rows {
                video_indices.push(cursor as u32);
                rows.push(row);
                cursor += 1;
            }
            rotary_time += advance;
        }
        let num_condition_video_rows = video_indices.len();
        let num_condition_audio_rows = audio_indices.len();
        ...
        audio_indices.extend((audio_start..video_start).map(|i| i as u32));
        video_indices.extend((video_start..seq_len).map(|i| i as u32));
        let text_indices: Vec<u32> = (0..num_text_tokens).map(|i| i as u32).collect();
