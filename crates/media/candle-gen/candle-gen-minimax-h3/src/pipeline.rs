...
///
/// Enforced by [`resolve_geometry`], not merely declared: `Capabilities::max_size` bounds each edge
/// at 1344 independently, so `1344 x 1344` would otherwise pass every check at 1.75x this budget.
/// Canvas area is the *dominant* term in the packed sequence — it sets rows per latent frame, and
/// attention is quadratic in the sequence — so an unbounded area is an unbounded render cost
/// regardless of what the duration bound says.
...
mod tests {
    ...
    ///
    /// `Capabilities::max_size` caps each edge at 1344 *independently*, so the square 1344×1344 is
