...
//! **model**, not of a backend, so it is restated rather than re-derived — and the two modules'
//! constants are asserted equal from a shared committed record in `tests/ref2va_cross_backend.rs`.
//!
