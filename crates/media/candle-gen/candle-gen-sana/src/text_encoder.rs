...
//! encoder (sc-11779, the Windows/CUDA + Linux sibling of mlx-gen-sana's sc-8488 / mlx-gen #614).
//!
...
//!
//! That is **byte-for-byte** [`candle_gen_pid::CaptionEncoder`]'s algorithm (the same encoder body,
//! padding-mask handling, last-hidden extraction, and [`candle_gen_pid::caption::select_index`]
//! token-selection policy that PiD uses). The two diverge in exactly **one** place: the CHI prompt
//! text. PiD's released `_CHI_PROMPT` wraps `Enhanced prompt` in escaped **double**-quotes; SANA's
...
//! [`candle_gen_pid::CaptionEncoder::with_chi_prompt`].
//!
//! Reuse seam: `candle-gen-sana` depends on `candle-gen-pid` and constructs a [`CaptionEncoder`] with
//! [`SANA_CHI_PROMPT`]. The Gemma-2 forward, padding-mask handling, and token-selection policy live
//! once, in `candle-gen-pid`; nothing is copy-pasted here.
//!
//! ## Mask handling
...
use candle_gen::{sorted_safetensors, Result, Weights};
use candle_gen_pid::{CaptionEncoder, Gemma2, Gemma2Config};

/// SANA's max caption sequence length (`max_sequence_length=300` in diffusers `SanaPipeline`). Also
/// the token count of the [`SanaTextEncoder::encode`] output's sequence axis (`[1, 300, 2304]`).
pub const MAX_SEQUENCE_LENGTH: i32 = 300;

/// The merged single-file gemma-2-2b-it checkpoint name (matches PiD's `GEMMA_MERGED_FILE` and the
/// un-gated `SceneWorks/gemma-2-2b-it` mirror layout).
...
/// diffusers `pipeline_sana.py` (matching NVlabs/Sana). Identical to PiD's
/// [`candle_gen_pid::caption::CHI_PROMPT`] template **except** `Enhanced prompt` is wrapped in
/// single-quotes here (PiD uses double-quotes); the difference is load-bearing because it changes the
/// tokenization. The user caption is appended directly after the trailing `"User Prompt: "`.
...
/// `attn2` cross-attention. A thin wrapper over the reused [`candle_gen_pid::CaptionEncoder`], wired
/// with [`SANA_CHI_PROMPT`].
...
impl SanaTextEncoder {
    ...
    /// `SceneWorks/gemma-2-2b-it` mirror) and the gemma `tokenizer.json` path. Uses SANA's
    /// [`SANA_CHI_PROMPT`] via the shared [`CaptionEncoder::with_chi_prompt`] seam — no encoder body
    /// is duplicated.
    ...
    /// Load gemma-2-2b-it from a snapshot directory and build the SANA text encoder. Prefers the
    /// merged single-file checkpoint (`<dir>/gemma-2-2b-it.safetensors`), else loads the snapshot's
    /// sharded `*.safetensors`. The gemma decoder runs f32 (the parity target + the dense-GEMM-safe
    /// path), mirroring [`candle_gen_pid::PidEngine::load`].
    pub fn from_snapshot(dir: impl AsRef<Path>, device: &Device) -> Result<Self> {
        let dir = dir.as_ref();
        let merged = dir.join(GEMMA_MERGED_FILE);
        let gw = if merged.is_file() {
            Weights::from_file(&merged, device, DType::F32)?
        } else {
            let files = sorted_safetensors(dir, "sana gemma text encoder")?;
