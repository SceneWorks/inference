...
//! `mlx-gen-sana`'s copies under the same names. Both lanes therefore load the same tensors through
...
use candle_gen_sana::SanaTransformerConfig;
...
/// to the MLX lane's — the two lanes load byte-identical golden bytes and must agree about their
