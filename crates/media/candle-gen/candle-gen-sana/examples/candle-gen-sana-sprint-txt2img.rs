...
//! explicitly registered CFG-free few-step generator through
//! `provider_registry().load("sana_sprint_1600m", …)`, runs a real `generate` against an
...
fn main() -> Result<()> {
    ...
    let gen = candle_gen_sana::provider_registry()?.load("sana_sprint_1600m", &spec)?;
    println!(
