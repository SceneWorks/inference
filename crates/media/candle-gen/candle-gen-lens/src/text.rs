//! Lens text input — the gpt-oss **o200k_harmony** tokenizer + the Lens chat template (sc-5109).
//!
...
//! assistant scaffold (sc-5110/sc-5112). The preamble's `Current date:` line is dynamic, so
//! [`LensTokenizer::encode`] takes the date as a parameter (the worker passes today's date; tests pass
//! the golden's). The preamble is always exactly [`TXT_OFFSET`] tokens, hence a fixed constant.
...
/// gpt-oss `pad_token_id`.
const PAD_TOKEN_ID: i32 = 199_999;
/// Harmony stop token id — the assistant turn terminates with `<|return|>` (the model's `eos`).
pub const HARMONY_RETURN: u32 = 200_002;
/// Harmony `<|end|>` — also terminates a message.
pub const HARMONY_END: u32 = 200_007;
...
impl LensTokenizer {
    ...
    pub fn from_file(tokenizer_json: impl AsRef<Path>) -> CResult<Self> {
        ...
        let inner = TextTokenizer::from_file(tokenizer_json, cfg)
            .map_err(|e| CandleError::Msg(format!("lens: load tokenizer: {e}")))?;
        Ok(Self { inner })
    }

    /// Tokenize `prompt` into `input_ids` (u32). `date` is the ISO `YYYY-MM-DD` for the harmony
    /// preamble. The DiT consumes `ids[TXT_OFFSET..]`.
    pub fn encode(&self, prompt: &str, date: &str) -> CResult<Vec<u32>> {
        self.inner
            .encode_ids(&render(prompt, date), true)
            .map(|ids| ids.into_iter().map(|i| i as u32).collect())
            .map_err(|e| CandleError::Msg(format!("lens: tokenize: {e}")))
    }

    /// Tokenize the **PromptReasoner** harmony prompt (sc-5118): the rewriter system instruction +
    /// `reasoning_effort="low"` + the trailing bare `<|start|>assistant` generation prompt (the model
    /// then emits its own `analysis`/`final` channel). Byte-identical to the vendor
    /// `PromptReasoner._refine_via_local` `apply_chat_template(..., add_generation_prompt=True,
    /// reasoning_effort="low")`.
    pub fn encode_reasoner(&self, prompt: &str, date: &str) -> CResult<Vec<u32>> {
        self.inner
            .encode_ids(&render_reasoner(prompt, date), true)
            .map(|ids| ids.into_iter().map(|i| i as u32).collect())
            .map_err(|e| CandleError::Msg(format!("lens: tokenize reasoner: {e}")))
    }

    /// Decode token ids back to text (special tokens kept — the harmony channel markers are needed by
    /// the reasoner's output cleaner, sc-5118).
    pub fn decode(&self, ids: &[u32]) -> CResult<String> {
        self.inner
            .decode(ids, false)
            .map_err(|e| CandleError::Msg(format!("lens: detokenize: {e}")))
    }
}
...
/// Render the harmony reasoner prompt (== `apply_chat_template([system, user],
/// add_generation_prompt=True, reasoning_effort="low")`). Same preamble as [`render`] but **`Reasoning:
/// low`**, the rewriter system instruction as the `developer` message, and the trailing bare
/// `<|start|>assistant` generation prompt (the model emits the channel itself).
...
mod tests {
    ...
    #[test]
    fn render_has_harmony_shape() {
        let r = render("a red cube on a wooden table", "2026-06-13");
        // System preamble (the ChatGPT identity), with the dynamic date line.
        assert!(r.starts_with(
            "<|start|>system<|message|>You are ChatGPT, a large language model trained by OpenAI."
        ));
        assert!(r.contains("Current date: 2026-06-13"));
        assert!(r.contains("\nReasoning: medium\n"));
        // The image-description instruction is the developer message.
        assert!(r.contains(
            "<|start|>developer<|message|># Instructions\n\nDescribe the image by detailing"
        ));
        // The user prompt, then the assistant analysis + final scaffold (no generation yet).
        assert!(r.contains("<|start|>user<|message|>a red cube on a wooden table<|end|>"));
        assert!(r.contains("<|start|>assistant<|channel|>analysis<|message|>Need to generate one image according to the description.<|end|>"));
        assert!(r.ends_with("<|start|>assistant<|channel|>final<|message|>"));
    }
    ...
    fn clean_analysis_only_is_empty() {
        // When the *cleaned* text begins with an `analysis` marker (no final channel ever emitted), it
        // carries no usable prompt → "" (the caller falls back to the original prompt).
