...
The regression is guarded by `candle-gen/tests/cuda_quant_smoke.rs`. **sc-19545 established that
this test had never run in CI** — it was documented as running in `scripts/check-cuda.ps1`, which no
workflow invokes, while the only automated CUDA lane compiles it with `--no-run` and discards the
binary. It now runs in the `candle-minimax-h3` real-weights job. That file also carries
`the_fatbin_covers_this_runners_gpu`, which reads the runner's real compute capability and checks it
against the `-gencode` ladder **parsed out of this `build.rs`**, so dropping a rung below fails a
test rather than silently narrowing the fatbin.

Two related facts, both now enforced:
...
* `CUDA_COMPUTE_CAP` must stay **80** at all 15 CI sites (13 in `real-weights.yml`, 2 in `ci.yml`).
  ...
  120 deletes the sm_80 rung here and lifts the dense PTX floor to `compute_120`. Held by
  `scripts/ci/cuda_arch_support.py` plus `test_every_cuda_job_declares_the_build_derived_compute_cap`,
  which DERIVE the expected value from this file rather than restating it.
* **Datacenter Blackwell sm_100 (B100/B200) is not covered** by the ladder below — major 10 is
  ...
  `build.rs` comment, but it means such a card would silently return zeros; the runtime guard fails
  loudly instead.
