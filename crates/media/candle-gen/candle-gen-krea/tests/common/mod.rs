...
//! `variant5_native_keys.txt` — is committed **byte-identical** to the file `mlx-gen-krea` commits
...
//! by name set and by value — so the module docs that used to assert "the SAME fixtures" in prose now
//! describe something enforced.
//!
//! `mod common;` is compiled into every including test binary, so only a subset is used by any one of
//! them.

use candle_gen_krea::{Krea2Config, KreaTeConfig};

// --- DiT fixtures: `tools/dump_krea_dit_golden.py` -----------------------------------------------

/// DiT attention heads.
pub const SHARED_FIXTURE_DIT_HEADS: usize = 4;
/// DiT key/value heads — GQA, strictly fewer than [`SHARED_FIXTURE_DIT_HEADS`].
pub const SHARED_FIXTURE_DIT_KV_HEADS: usize = 2;
/// DiT per-head width. `mmdit` derives the 3-axis RoPE dims `[8, 12, 12]` from this 32.
pub const SHARED_FIXTURE_DIT_HEAD_DIM: usize = 32;
/// DiT residual width.
pub const SHARED_FIXTURE_DIT_HIDDEN: usize = 128;
/// Text-fusion attention heads (both the layerwise and the refiner blocks).
pub const SHARED_FIXTURE_DIT_TXT_HEADS: usize = 2;
/// Norm epsilon shared by the block, text-fusion and full-DiT fixtures.
pub const SHARED_FIXTURE_DIT_EPS: f64 = 1e-5;
/// DiT latent channels.
...
pub const SHARED_FIXTURE_TE_HIDDEN_SIZE: usize = 64;
/// TE decoder depth.
pub const SHARED_FIXTURE_TE_NUM_LAYERS: usize = 6;
/// TE attention heads.
pub const SHARED_FIXTURE_TE_NUM_HEADS: usize = 4;
/// TE key/value heads (GQA).
pub const SHARED_FIXTURE_TE_NUM_KV_HEADS: usize = 2;
/// TE per-head width — decoupled from `hidden_size`, as in the shipped Qwen3-VL-4B.
pub const SHARED_FIXTURE_TE_HEAD_DIM: usize = 32;
/// TE RMSNorm epsilon.
pub const SHARED_FIXTURE_TE_RMS_NORM_EPS: f64 = 1e-6;
/// TE RoPE base.
...
pub const SHARED_FIXTURE_TE_IMAGE_TOKEN_ID: u32 = 151_655;
...
/// `forward` both lanes' `te_golden` tests drive builds a plain 1-D RoPE table and never looks at it.
/// The MLX lane used to declare a reduced `[16, 0, 0]` here, saturated to head_dim/2, under a comment
/// explaining that text-only MRoPE collapses to the T axis — true, but a second spelling of a
/// production constant that nothing held equal to the other lane's. Both lanes now carry the shipped
/// `[24, 20, 20]`, which at head_dim 32 puts the same 16 frequencies on the same axis.
pub const SHARED_FIXTURE_TE_MROPE_SECTION: [usize; 3] = [24, 20, 20];
...
pub fn tiny_dit_config() -> Krea2Config {
    Krea2Config {
        ...
        hidden_size: SHARED_FIXTURE_DIT_HIDDEN,
        num_attention_heads: SHARED_FIXTURE_DIT_HEADS,
        num_kv_heads: SHARED_FIXTURE_DIT_KV_HEADS,
        attention_head_dim: SHARED_FIXTURE_DIT_HEAD_DIM,
