//! Ideogram 4's logit-normal flow-matching schedule + Euler sampler timing (upstream
//! `ideogram4.scheduler`). The published `model_index.json` names `FlowMatchEulerDiscreteScheduler`,
//! but the real `Ideogram4Pipeline` samples with this resolution-aware `LogitNormalSchedule`.
//!
//! `schedule(t) = clamp(1 − σ(mean + std·Φ⁻¹(t)), t_min, t_max)` where `mean` grows with the image
//! pixel count, `Φ⁻¹` is the probit (inverse normal CDF), `σ` is the logistic, and the clamp
//! bounds come from the logSNR range. The denoise loop walks `make_step_intervals(num_steps)`
//! (linear `[0,1]`) from high noise to low, Euler-stepping `z += v·(s−t)` per step.

/// Logit-normal noise schedule for one resolution. `mean`/`std` are the logit-normal params;
/// the clamp bounds come from `logsnr_min=-15`, `logsnr_max=18`.
...
impl LogitNormalSchedule {
    ...
    /// Map a step interval `t ∈ [0,1]` to a flow-matching time.
...
/// Inverse normal CDF (probit), Acklam's rational approximation (|err| ≲ 1.15e-9). Endpoints map
/// to ±∞ so the schedule clamp produces the boundary `t_min`/`t_max` (matching torch `ndtri`).
fn ndtri(p: f64) -> f64 {
    if p <= 0.0 {
