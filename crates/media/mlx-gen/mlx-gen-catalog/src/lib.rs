...
/// Resolve a provider-owned, load-exact numeric tier for MLX video admission. Bernini validates its
/// paired planner/renderer packed manifests and headers; Krea and Wan read their immutable snapshot
/// surfaces. Other routes return `None` rather than synthesizing a tier.
