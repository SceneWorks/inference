...
fn controlnet_snapshot() -> PathBuf {
    let p = std::env::var("KOLORS_CONTROLNET").unwrap_or_else(|_| panic!("set KOLORS_CONTROLNET to the required snapshot dir; inference never self-fetches or derives a cache location (epic 13657)"));
    PathBuf::from(p)
}
