...
//!
//! The audio VAE ships in two published forms with **identical tensor names** (1087 each, verified
//! by [`crate::audio_vae`]'s real-weight smoke) but different configs:
//!
//! | | `FL2VA/audio_vae/` | root `audio_vae/` |
//! |---|---|---|
//! | class | `MiniMaxH3AudioVAE` (remote code) | `AutoencoderKLMiniMaxH3Audio` (diffusers 0.36) |
...
//!   [`BigVganConfig::for_sample_rate`]'s caller asserts they do — a checkpoint where they diverged
//!   would otherwise decode at the wrong rate with no error.
...
pub const DEFAULT_ENCODER_DIM: i32 = 64;
...
pub const ACTIVATION_RESAMPLE_RATIO: i32 = 2;
...
pub const ACTIVATION_KERNEL_SIZE: i32 = 12;
...
impl BigVganConfig {
    ...
    pub fn for_sample_rate(sample_rate: u32, num_mels: i32, decoder_dim: i32) -> Result<Self> {
    ...
    pub fn stage_in_channels(&self, stage: usize) -> i32 {
        self.upsample_initial_channel / (1 << stage)
    }

    /// Channel width leaving stage `i` — also the AMP-block width for that stage.
    pub fn stage_out_channels(&self, stage: usize) -> i32 {
        self.upsample_initial_channel / (1 << (stage + 1))
    }
...
pub struct MiniMaxH3AudioVaeConfig {
    ...
    /// `metadata.json` `kwargs.encoder_dim`; also the constructor default `latent_dim` derives
    /// from.
    pub encoder_dim: i32,
    /// `metadata.json` `kwargs.encoder_rates` — the encode half is not ported, but its **length**
    /// sets `latent_dim`.
    pub encoder_rates: Vec<i32>,
...
impl Default for MiniMaxH3AudioVaeConfig {
    ...
    fn default() -> Self {
        ...
        let latent_dim = DEFAULT_ENCODER_DIM * (1 << encoder_rates.len());
        Self {
            sample_rate: AUDIO_SAMPLE_RATE,
            output_channels: AUDIO_OUTPUT_CHANNELS,
            latent_channels: AUDIO_LATENT_CHANNELS as i32,
...
impl MiniMaxH3AudioVaeConfig {
    ...
    ///
    /// Every constructor argument is taken from the file the reference takes it from — `config.yaml`
    /// for `decoder_dim` / `vae_latent_channels`, `metadata.json` for the rest, `config.json` for
    /// `output_channel` and the latent statistics. Nothing here is hardcoded except the
    /// `sample_rate`-selected BigVGAN block, which the reference itself hardcodes.
    pub fn from_source_files(
        ...
        let config: serde_json::Value = serde_json::from_str(config_json)
            .map_err(|e| Error::Msg(format!("minimax-h3 audio vae config.json: {e}")))?;
        let metadata: serde_json::Value = serde_json::from_str(metadata_json)
            .map_err(|e| Error::Msg(format!("minimax-h3 audio vae metadata.json: {e}")))?;
        let kwargs = metadata
            .get("metadata")
            .and_then(|m| m.get("kwargs"))
            .ok_or_else(|| {
                Error::Msg("minimax-h3 audio vae metadata.json: no metadata.kwargs".into())
            })?;
        ...
        let sample_rate = u32::try_from(int_of(kwargs, "sample_rate", "metadata.kwargs")?)
            .map_err(|_| Error::Msg("minimax-h3 audio vae: negative sample_rate".into()))?;
        ...
        let decoder_dim = i32::try_from(yaml_int(&model_config, "decoder_dim")?)
            .map_err(|_| Error::Msg("minimax-h3 audio vae: decoder_dim out of range".into()))?;
        let latent_channels = i32::try_from(yaml_int(&model_config, "vae_latent_channels")?)
            .map_err(|_| {
                Error::Msg("minimax-h3 audio vae: vae_latent_channels out of range".into())
            })?;

        let encoder_rates = int_vec(kwargs, "encoder_rates", "metadata.kwargs")?;
        let decoder_rates = int_vec(kwargs, "decoder_rates", "metadata.kwargs")?;
        let decoder_type = kwargs
            .get("decoder_type")
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| {
                Error::Msg("minimax-h3 audio vae metadata.kwargs: no decoder_type".into())
            })?
            .to_string();
        ...
        let attn_proj = kwargs
            ...
            .ok_or_else(|| {
                Error::Msg("minimax-h3 audio vae metadata.kwargs: no attn_proj".into())
            })?;
        ...
        if let Ok(declared) = int_of(kwargs, "encoder_dim", "metadata.kwargs") {
            if declared != i64::from(encoder_dim) {
                return Err(Error::Msg(format!(
        ...
        if let Ok(declared) = int_of(kwargs, "latent_dim", "metadata.kwargs") {
            if declared != i64::from(latent_dim) {
                return Err(Error::Msg(format!(
        ...
        for (key, from_yaml) in [
            ("decoder_dim", i64::from(decoder_dim)),
            ("vae_latent_channels", i64::from(latent_channels)),
        ] {
        ...
        let output_channels = u16::try_from(int_of(&config, "output_channel", "config.json")?)
            .map_err(|_| Error::Msg("minimax-h3 audio vae: output_channel out of range".into()))?;
    ...
    pub fn cross_check_diffusers_json(&self, config_json: &str) -> Result<()> {
        let root: serde_json::Value = serde_json::from_str(config_json)
            .map_err(|e| Error::Msg(format!("minimax-h3 audio vae root config.json: {e}")))?;
        ...
        let checks: [(&str, i64, i64); 4] = [
            (
                ...
                i64::from(self.latent_channels),
            ),
            (
                "latent_dim",
                int_of(&root, "latent_dim", ctx)?,
                i64::from(self.bigvgan.num_mels),
            ),
            (
                "decoder_dim",
                int_of(&root, "decoder_dim", ctx)?,
                i64::from(self.decoder_dim),
            ),
            (
                "encoder_dim",
                int_of(&root, "encoder_dim", ctx)?,
                i64::from(self.encoder_dim),
            ),
        ];
        ...
        let vectors: [(&str, Vec<i32>, &Vec<i32>); 4] = [
            (
                "decoder_rates",
                int_vec(&root, "decoder_rates", ctx)?,
                &self.decoder_rates,
            ),
            (
                "decoder_kernel_sizes",
                int_vec(&root, "decoder_kernel_sizes", ctx)?,
                &self.bigvgan.upsample_kernel_sizes,
            ),
            (
                "encoder_rates",
                int_vec(&root, "encoder_rates", ctx)?,
                &self.encoder_rates,
            ),
            (
                "resblock_kernel_sizes",
                int_vec(&root, "resblock_kernel_sizes", ctx)?,
...
fn derive_latent_dim(encoder_dim: i32, encoder_rate_count: usize) -> Result<i32> {
    if encoder_rate_count >= 24 {
        return Err(Error::Msg(format!(
            "minimax-h3 audio vae: {encoder_rate_count} encoder_rates overflow latent_dim"
        )));
    }
    Ok(encoder_dim * (1 << encoder_rate_count))
}

fn int_of(v: &serde_json::Value, key: &str, ctx: &str) -> Result<i64> {
    v.get(key)
        .and_then(serde_json::Value::as_i64)
        .ok_or_else(|| Error::Msg(format!("minimax-h3 audio vae {ctx}: no integer `{key}`")))
}

fn int_vec(v: &serde_json::Value, key: &str, ctx: &str) -> Result<Vec<i32>> {
    v.get(key)
        .and_then(serde_json::Value::as_array)
        .ok_or_else(|| Error::Msg(format!("minimax-h3 audio vae {ctx}: no array `{key}`")))?
        .iter()
        .map(|e| {
            e.as_i64()
                ...
                .ok_or_else(|| {
                    Error::Msg(format!(
                        "minimax-h3 audio vae {ctx}: `{key}` has a non-integer"
                    ))
                })
        })
        .collect()
...
mod tests {
    ...
    fn shipped_geometry_is_the_published_one() {
        ...
        assert_eq!(cfg.token_rate_hz(), AUDIO_TOKEN_RATE_HZ as f32);
