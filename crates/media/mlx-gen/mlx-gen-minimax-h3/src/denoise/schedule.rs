...
//! [`JointSchedule::new_from_parts`] can reject a swapped pair. `swapping_the_shifts_is_observable`
//! and `applying_one_shift_to_both_modalities_is_observable` pin that the distinction is
//! measurable, not merely typed.
//!
...
/// Video sigma shift — `_minimax_h3.sigma_shift_scales.video`, and `scheduler/scheduler_config.json`.
...
///
/// Only `ref2va` (sc-17149) has such rows; `t2va` / `fl2va` carry none.
...
impl DenoiseModality {
    ...
    pub const fn token_tag(self) -> i32 {
        match self {
            Self::Video => 0,
            Self::Audio => 2,
        }
...
/// same as applying it once with any single shift — `shift(shift(σ, s), s) = shift(σ, s²/(2s − 1))`
/// only for the composition of the *Möbius* form, and the arithmetic is observably different at
/// every interior point; `double_applying_a_shift_is_observable` pins it.
...
///
/// Verified bitwise against `torch.linspace` for every `N` in `2..400`.
...
impl SigmaSchedule {
    ...
    /// Adopt an explicit, already-shifted grid — the reference's `sigmas=` path, which applies **no**
    /// shift and **no** collapse.
    ///
    ...
    /// ```
    pub fn step(&self, index: usize, sample: &Array, velocity: &Array) -> Result<Array> {
        ...
        let x = sample.as_dtype(Dtype::Float32)?;
        let v = velocity.as_dtype(Dtype::Float32)?;
        ...
        let denoised = add(&x, &multiply(&v, Array::from_f32(sigma_from_timestep))?)?;

        // ...and the Euler ratio from the stored GRID.
        let ratio = self.sigmas[index + 1] / self.sigmas[index];
        Ok(add(
            &multiply(&x, Array::from_f32(ratio))?,
            &multiply(&denoised, Array::from_f32(1.0 - ratio))?,
        )?)
    }
}
...
mod tests {
    ...
    fn the_velocity_sign_is_data_ward() {
        ...
        let x = Array::from_slice(&[0.0f32, 0.0], &[2]);
        let v = Array::from_slice(&[1.0f32, -1.0], &[2]);
        let out = s.step(1, &x, &v).unwrap();
    ...
    fn the_last_step_returns_the_denoised_sample() {
        ...
        let v = Array::from_slice(&[0.5f32], &[1]);
        let got: f32 = s.step(last, &x, &v).unwrap().item();
        let want = 2.0 + (1.0 - s.timesteps()[last]) * 0.5;
        assert!((got - want).abs() < 1e-6, "{got} vs {want}");
        assert!(s.step(last + 1, &x, &v).is_err(), "past the end");
        assert!(
            s.step(0, &x, &Array::from_slice(&[1.0f32, 2.0], &[2]))
                .is_err(),
            "mismatched rows"
        );
    }
