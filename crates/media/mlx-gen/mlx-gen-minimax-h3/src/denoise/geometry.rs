...
//! side corrected — see `mlx_gen_minimax_h3::pipeline::fit_audio_to_video` for why. Stating the two
//! counts *here* is what lets the candle lane inherit the same policy instead of re-deriving one
//! (`scripts/check-workspace.py::check_cross_backend_geometry` compares every `pub const` in this
//! file across both crates), and it is why the delivered length is a function of `num_frames` alone
//! and never of what came back from the decoder.
...
pub fn av_grids_align_exactly(num_frames: i32) -> bool {
    (num_frames * AUDIO_LATENTS_PER_SECOND).rem_euclid(MINIMAX_H3_FPS as i32) == 0
}
...
pub fn decoded_audio_samples(num_frames: i32) -> i32 {
    audio_latent_num_frames(num_frames) * AUDIO_SAMPLES_PER_LATENT as i32
}
...
pub fn delivered_audio_samples(num_frames: i32) -> i32 {
    (f64::from(num_frames) / MINIMAX_H3_FPS * f64::from(AUDIO_SAMPLE_RATE)).round() as i32
}
...
mod tests {
    ...
    fn the_exactly_aligned_durations_are_derived_not_tabulated() {
        ...
        for &frames in &LEGAL_FRAME_COUNTS {
            ...
            let round_tripped = audio_latent_num_frames(frames) * MINIMAX_H3_FPS as i32
                == frames * AUDIO_LATENTS_PER_SECOND;
            assert_eq!(
    ...
    fn the_delivered_soundtrack_length_is_the_pictures_at_every_legal_count() {
        ...
        let mut corrected = 0;
        for &frames in &LEGAL_FRAME_COUNTS {
            let picture = f64::from(frames) / MINIMAX_H3_FPS;
            let delivered = delivered_audio_samples(frames);
            let residual = f64::from(delivered) / f64::from(AUDIO_SAMPLE_RATE) - picture;
            assert!(
            ...
            let correction = f64::from(decoded - delivered) / f64::from(AUDIO_SAMPLE_RATE);
            assert!(
