...
//!
//! # Row classes, and a correction to what sc-17145 recorded
//!
...
//! the three assignments, so they keep the default fill. [`crate::dit::adaln`]'s module docs and
//! the sc-17242 spike comment both describe them as sitting at a clean `1.0`; that is wrong, and
//! `text_rows_carry_the_video_timestep` pins the correct reading against the reference's own
//! fixture. The `1.0` class is real but belongs to `ref2va` reference-audio rows, which `t2va` and
//! `fl2va` have none of.
//!
//! It does not change the arithmetic of the AdaLN cache — [`crate::dit::adaln::TimestepSchedule`]
//! is agnostic to what the classes mean — but it does change which rows are addressed by which
//! table block, which is a silent output change and not a cosmetic one.
//!
...
pub const VIDEO_TAG: i32 = 0;
/// `MINIMAX_H3_TEXT_TAG`.
pub const TEXT_TAG: i32 = 1;
/// `MINIMAX_H3_AUDIO_TAG`.
pub const AUDIO_TAG: i32 = 2;
...
impl PackedLayout {
    ...
    pub fn build(
        ...
        let num_condition_video_rows = keyframe_anchors.len() as i32 * rows_per_frame;
        // `t2va` / `fl2va` carry no reference soundtrack; `ref2va` (sc-17149) is the only
        // presentation with leading audio conditioning rows.
        let num_condition_audio_rows = 0i32;
        ...
        let video_indices: Vec<i32> = (condition_start..audio_start)
        ...
        for (row, &tag) in text_indices.iter().zip(text_token_tags) {
            ...
            classes[*row as usize] = RowClass::Video as i32;
        }
        for (i, &row) in audio_indices.iter().enumerate() {
            tags[row as usize] = AUDIO_TAG;
            classes[row as usize] = if (i as i32) < num_condition_audio_rows {
                RowClass::ConditionAudio as i32
            } else {
                RowClass::Audio as i32
            };
        }
        for (i, &row) in video_indices.iter().enumerate() {
            tags[row as usize] = VIDEO_TAG;
            classes[row as usize] = if (i as i32) < num_condition_video_rows {
                RowClass::ConditionVideo as i32
            } else {
                RowClass::Video as i32
            };
        }
        ...
        Ok(Self {
            ...
            seq_len,
            position_ids,
            token_tags: Array::from_slice(&tags, &[seq_len]),
            row_classes: Array::from_slice(&classes, &[seq_len]),
            video_indices,
            audio_indices,
            text_indices,
            inverse,
    ...
    /// Text rows.
    pub fn num_text_tokens(&self) -> i32 {
        self.num_text_tokens
    }

    /// Patched rows one latent frame contributes.
    pub fn rows_per_frame(&self) -> i32 {
        self.rows_per_frame
    }
    ...
    /// through the layout's own index tensors, so a wrong index tensor produces a wrong sequence
    /// rather than a coincidentally-right concatenation.
    pub fn scatter(&self, text: &Array, video: &Array, audio: &Array) -> Result<Array> {
        ...
        let sources = concatenate_axis(&[text.clone(), video.clone(), audio.clone()], 1)?;
        let perm = Array::from_slice(&self.inverse, &[self.seq_len]);
    ...
    pub fn gather(&self, packed: &Array, rows: &[i32]) -> Result<Array> {
        let s = packed.shape();
    ...
    pub fn generated_video_rows(&self) -> &[i32] {
        &self.video_indices[self.num_condition_video_rows as usize..]
    }

    /// The generated tail of an audio row block.
    pub fn generated_audio_rows(&self) -> &[i32] {
        &self.audio_indices[self.num_condition_audio_rows as usize..]
    }

    fn check_block(&self, x: &Array, rows: usize, what: &str) -> Result<i32> {
        let s = x.shape();
...
mod tests {
    ...
    pub(super) const TEST_FRAMES: i32 = 124;
    ...
    fn the_row_order_is_text_conditions_audio_video() {
        ...
        assert_eq!(l.text_indices(), (0..5).collect::<Vec<_>>());
        assert_eq!(l.audio_indices(), (5..419).collect::<Vec<_>>());
        assert_eq!(l.video_indices(), (419..641).collect::<Vec<_>>());
        ...
        assert_eq!(k.audio_indices(), (17..431).collect::<Vec<_>>());
    }
    ...
    fn video_indices_are_discontiguous_across_the_audio_block() {
        ...
        assert_eq!(&v[..12], &(5..17).collect::<Vec<_>>()[..], "the anchors");
        assert_eq!(&v[12..], &(431..653).collect::<Vec<_>>()[..], "the targets");
        assert!(
            v[11] + 1 != v[12],
    ...
    /// `build_row_timesteps`. This is the correction to what sc-17145's docs recorded.
    #[test]
    fn text_rows_carry_the_video_timestep() {
        let k = layout(&[KeyframeAnchor::First]);
        let classes = k.row_classes().as_slice::<i32>();
        ...
        assert!(
            !classes.contains(&(RowClass::ConditionAudio as i32)),
    ...
    fn tags_and_classes_are_assigned_per_row() {
        let k = layout(&[KeyframeAnchor::First]);
        let tags = k.token_tags().as_slice::<i32>();
        let classes = k.row_classes().as_slice::<i32>();
        for &row in k.text_indices() {
        ...
        for (i, &row) in k.video_indices().iter().enumerate() {
            ...
            let want = if (i as i32) < k.num_condition_video_rows() {
                RowClass::ConditionVideo
            } else {
                RowClass::Video
            };
            assert_eq!(classes[row as usize], want as i32);
        }
        assert!(tags.iter().all(|t| (0..MODALITY_NUM).contains(t)));
    }
    ...
    fn scatter_and_gather_round_trip() {
        ...
        let block = |n: usize, base: f32| {
            let v: Vec<f32> = (0..n * f as usize).map(|i| base + i as f32).collect();
        ...
        assert_eq!(packed.shape(), &[1, k.seq_len(), f]);
        ...
        let flat = packed.as_slice::<f32>();
        assert_eq!(flat[0], 1000.0);
        let audio_start = k.audio_indices()[0] as usize * f as usize;
        assert_eq!(flat[audio_start], 3000.0);
    ...
    fn the_layout_drives_an_adaln_schedule() {
        ...
        for step in 0..schedule.num_steps() {
            let idx = schedule
                .adaln_indices(step, k.row_classes(), k.token_tags())
                .unwrap();
            assert_eq!(idx.shape(), &[k.seq_len()]);
            let max: i32 = idx.max(None).unwrap().item();
            assert!(max < schedule.modulation_rows());
            ...
            assert_eq!(
                flat[text_row] - TEXT_TAG,
                flat[target_row] - VIDEO_TAG,
    ...
    fn malformed_layouts_are_rejected() {
        ...
        assert!(
            PackedLayout::build(g, [1, 2, 2], &[], 2, &[]).is_err(),
            "no text"
        );
        assert!(
            PackedLayout::build(g, [1, 2, 2], &[9], 2, &[]).is_err(),
            "tag out of range"
        );
        assert!(
            PackedLayout::build(g, [1, 2, 2], &[TEXT_TAG], 0, &[]).is_err(),
            "no audio channels"
        );
        assert!(
            PackedLayout::build(g, [1, 5, 5], &[TEXT_TAG], 2, &[]).is_err(),
        ...
        assert!(
            PackedLayout::build(drifted, [1, 2, 2], &[TEXT_TAG], 2, &[]).is_err(),
