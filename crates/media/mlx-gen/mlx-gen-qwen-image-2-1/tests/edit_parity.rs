...
//! are held near exact). Worst measured across every case, this fixture:
//!
//! * `pixel_values` **2.384e-7** at `peak` 2.146 (one f32 ULP), `vae_input` **exactly 0** —
//!   bound **1e-5 × peak** (host `u8 → f32` arithmetic);
//! * `prompt_embeds` **4.160e-3** at `peak` 2.795 = `1.5e-3 × peak` (`annotated`) — bound
//!   **2e-2 × peak**, the same bar the text-encoder gate holds, over a longer graph (ViT tower +
//!   DeepStack + interleaved M-RoPE + the decoder), ~13× headroom;
//! * reference latents **3.207e-4** (`aspect/ref_latents_2`, peak 0.655) and end-to-end latents
//!   **1.747e-2** at `peak` 5.206 = `3.4e-3 × peak` (`ref1`) — bound **2.5e-2 × peak**, the bar
//!   `pipeline_parity` already holds the text-to-image denoise to, ~7× headroom.
//!
//! The reference-ORDER claim is deliberately **not** made against any of these: `LATENT_TOL ×
...
/// own evidence (measured 1.083e-3 at peak 0.841) rather than inheriting [`TEXT_TOL`]: that bar
/// is set for the reference route's much longer graph, and at this peak it is loose enough to
/// pass a visibly wrong text path. ~3.7x headroom.
const T2I_ORACLE_TOL: f32 = 4e-3;
...
/// (`tools/dump_qwen21_text_encoder.py`): `(tensor key, the prompt it was dumped for)`.
const FIXTURE_PROMPT_ORACLE: (&str, &str) = ("fox/prompt_embeds", "a red fox in the forest");

fn fixture_text_encoder() -> Weights {
    fixture("qwen21_text_encoder.safetensors")
}

fn te_tensor(w: &Weights, key: &str) -> Array {
    w.require(key).expect("text-encoder oracle tensor").clone()
}

/// `[1, L, hidden]` -> `[L, hidden]`, the shape the S1 oracle stores (`embeds[0]`).
fn squeeze_batch(a: &Array) -> Array {
    a.squeeze_axes(&[0]).expect("a single-sample conditioning")
}
...
/// Which order the condition latents reach the denoiser in. `Reversed` is a deliberately wrong
/// feed used by [`swapping_two_references_changes_the_output`]: the joint layout, the text
/// conditioning and the per-reference RoPE blocks all stay put, only the latents change places,
/// which is precisely the defect a magnitude-only ordering check cannot see.
...
fn run_case_feeding(snap: &Snapshot, w: &Weights, c: &Case, feed: ReferenceFeed) -> Array {
    ...
    let mut reference_latents = encode_references(&snap.vae, &prepared).unwrap();
