...
/// (`tests/generate_smoke.rs::decode_tiling_sweep_against_single_pass` — that harness arrived with
/// sc-8446 S13 and is NOT on this release line, so the numbers below are cited from the upstream run
/// rather than reproducible here):
///
...
///    spatial tile (`tests/generate_smoke.rs::decode_tiling_sweep_against_single_pass` — that harness arrived with
/// sc-8446 S13 and is NOT on this release line, so the numbers below are cited from the upstream run
/// rather than reproducible here):
///
