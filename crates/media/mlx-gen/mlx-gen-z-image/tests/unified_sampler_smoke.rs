...
fn snapshot() -> PathBuf {
    let p = std::env::var("ZIMAGE_SNAPSHOT").unwrap_or_else(|_| panic!("set ZIMAGE_SNAPSHOT to the required snapshot dir; inference never self-fetches or derives a cache location (epic 13657)"));
    PathBuf::from(p)
}
