...
- **`mlx-gen-<family>`** (30 crates) — provider crates: `-z-image`, `-flux`, `-flux2`, `-chroma`, `-qwen-image`, `-qwen-image-2-1`, `-krea`, `-sdxl`, `-kolors`, `-sensenova`, `-wan`, `-ltx`, `-svd`, `-seedvr2`, `-pulid`, `-instantid`, `-face`, `-joycaption`, `-sam2`, `-sam3`, `-bernini`, `-scail2`, `-lens`, `-boogu`, `-ideogram`, `-sd3`, `-sana`, `-depth`, `-pid`, `-clip`. Most depend **only on `mlx-gen`** and publish named registrations through a `register_providers` builder. The reuse map (sibling crate deps, NOT core-only) is now extensive — keep it straight before editing a reused crate:
  - `-qwen-image-2-1` (sc-24108) is a **distinct architecture** from `-qwen-image` (2512), not an alias: single-stream block-causal DiT, Qwen3-VL text tower, 64-channel RGBA VAE. It reuses `-z-image`'s Qwen3 `text_encoder::EncoderLayer` for its language tower. Its parity fixtures come from the pinned **diffusers** `QwenImage21Pipeline` (`tools/dump_qwen21_*.py`, a torch venv — not the mflux fork), and its tiny snapshot under `tests/fixtures/tiny-snapshot/` drives the full generator contract weights-free.
...
cargo test -p mlx-gen-z-image --test integration pipeline::   # one test file (tests/pipeline.rs)
...
- `mlx-rs` is pinned to a **GitHub fork by commit SHA** (`pmetal-mlx-rs`, mlx-rs 0.25 whose mlx-sys builds MLX core 0.32.0 as of epic 12742). Read the long comments in `Cargo.toml` before touching the SHA.
- **`MACOSX_DEPLOYMENT_TARGET = "26.2"`** in `.cargo/config.toml` is load-bearing: below 26.2 the NAX 16-bit Metal kernels miscompile to garbage (bf16/f16 GEMM + fused SDPA). CI lowers it to 15.0 because hosted runners cap at SDK 15 (so CI does NOT exercise the NAX fast path — a macOS-26.2+ self-hosted runner is required for that). Since the sc-21382 fork rev, `mlx-sys`'s `build.rs` declares `rerun-if-env-changed` on it, so changing the target re-runs the build script (before that a clean rebuild of `pmetal-mlx-sys` was required).
...
- **Prebuilt libmlx (sc-21382).** `pmetal-mlx-sys`'s `build.rs` otherwise runs a ~6 min cmake build of MLX (plus a network fetch) on every cold build — every fresh worktree, every `cargo clean`, every CI cache miss. The fork (`SceneWorks/mlx-rs`, workflow `prebuilt-mlx`) publishes that build per commit as release `prebuilt-<sha12>`, one tarball per cell (target × `MACOSX_DEPLOYMENT_TARGET` × Debug/Release). `scripts/fetch-prebuilt-mlx.sh` fetches the cell for the rev pinned in `Cargo.lock` into `~/.cache/pmetal/prebuilt/` and prints `PMETAL_MLX_PREBUILT_DIR=…`; with that exported, `build.rs` verifies the manifest against its own key (source fingerprint, target, deployment target, features, build type) and links the archives — **a mismatch fails the build, it never links silently**. It also prints `PMETAL_METALLIB_PATH=<dir>/mlx.metallib` (sc-20799): consuming a prebuilt — or merely hitting a warm cargo cache — means `build.rs` does not re-run, so it never refreshes `~/.cache/pmetal/lib/mlx.metallib`, and a `cargo test` binary (no compiled-in `METAL_PATH`) then dies at *"Failed to load the default metallib"*; the tarball ships the metallib, so point the resolver's first entry straight at it. Local: `eval "$(scripts/fetch-prebuilt-mlx.sh)" && export PMETAL_MLX_PREBUILT_DIR PMETAL_METALLIB_PATH` (Debug, 26.2 by default; `--build-type Release` for `--release`). CI: the `macos-metal` and `macos-nax` jobs run the script; "no prebuilt for this rev" is a tolerated, annotated fallback to source. **When bumping the `mlx-rs` rev**: the fork's `prebuilt-mlx` workflow runs on every push to its `main`, so prebuilts exist for any merge commit — confirm `https://github.com/SceneWorks/mlx-rs/releases/tag/prebuilt-<sha12>` has the 4 assets before (or soon after) the bump; until then hosted CI pays the cmake build once per cache miss, loudly.
- **Local metallib cache can shadow the build (sc-7889).** The fork loads its Metal kernel library at runtime via a patched resolver (`mlx-sys/patches/metallib-search-path.patch`). After sc-7898 the resolution order is: `$PMETAL_METALLIB_PATH` → **the build's own compiled-in metallib (METAL_PATH)** → **`~/.cache/pmetal/lib/mlx.metallib`** (the user cache) — i.e. own-build wins over the user cache (this reversed the pre-sc-7898 order, in which the user cache could shadow an incomplete build). Local `cargo test`/`run` binaries have **no compiled-in metallib**, so the user-cache copy is the *sole* working resolution for them — load-bearing, never delete it without replacing it. `build.rs` only refreshes that cache when it re-runs **and** the build artifact is mtime-newer ("newest-build-wins by mtime"); it is keyed by neither rev nor content. So an **older/incomplete** pmetal build (debug/release × historical rev) can own the cache and shadow a complete build. Older revs instantiate a smaller steel-GEMM NAX tile set lacking the small-M `bm64` tile (`steel_gemm_fused_nax_*_bm64_bn128_bk256_*`): large-M matmuls (inference) pick tiles the cache *has* and work, but small-M matmuls (**every LoRA training step**, M≈64) pick `bm64` → **PANIC "… _bm64_… not found"**. Not a missing-kernel/JIT issue — the pinned fork's metal source *does* instantiate `bm64`; the fault is the stale cache. Cure/guard: `tools/refresh_pmetal_metallib.sh` (installs the newest complete build metallib into the cache; `--check` validates it — run as a preflight before local real-weight training). Standing tripwire in the suite: `cargo test -p mlx-gen-qwen-image --release --test integration bf16_matmul_sweep::` (exercises small-M NAX GEMM; crashes if the cache regresses).
