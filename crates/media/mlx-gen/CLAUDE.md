...
- **Prebuilt libmlx (sc-21382).** `pmetal-mlx-sys`'s `build.rs` otherwise runs a ~6 min cmake build of MLX (plus a network fetch) on every cold build — every fresh worktree, every `cargo clean`, every CI cache miss. The fork (`SceneWorks/mlx-rs`, workflow `prebuilt-mlx`) publishes that build per commit as release `prebuilt-<sha12>`, one tarball per cell (target × `MACOSX_DEPLOYMENT_TARGET` × Debug/Release). `scripts/fetch-prebuilt-mlx.sh` fetches the cell for the rev pinned in `Cargo.lock` into `~/.cache/pmetal/prebuilt/` and prints `PMETAL_MLX_PREBUILT_DIR=…`; with that exported, `build.rs` verifies the manifest against its own key (source fingerprint, target, deployment target, features, build type) and links the archives — **a mismatch fails the build, it never links silently**. Local: `eval "$(scripts/fetch-prebuilt-mlx.sh)" && export PMETAL_MLX_PREBUILT_DIR` (Debug, 26.2 by default; `--build-type Release` for `--release`). CI: the `macos-metal` and `macos-nax` jobs run the script; "no prebuilt for this rev" is a tolerated, annotated fallback to source. **When bumping the `mlx-rs` rev**: the fork's `prebuilt-mlx` workflow runs on every push to its `main`, so prebuilts exist for any merge commit — confirm `https://github.com/SceneWorks/mlx-rs/releases/tag/prebuilt-<sha12>` has the 4 assets before (or soon after) the bump; until then hosted CI pays the cmake build once per cache miss, loudly.
