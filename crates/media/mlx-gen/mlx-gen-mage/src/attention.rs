...
//!   default softmax scale. The reference expresses that order as scatter offsets rather than a
//!   `cat`: text lands at each sample's start and image is shifted by that sample's text length
//!   (`:456-457`), and the scatter that consumes those offsets is at `:470-475`; the `torch.cat`
//!   at `:425-427` is commented out. Per-sample isolation is varlen `cu_seqlens`, not a
//!   block-diagonal mask (MLX has no varlen flash kernel — the port picks its own equivalent, and
//!   must keep the isolation exact because native-resolution packing puts unrelated samples in one
//!   sequence).
//!
