...
fn registered_descriptors_conform() {
    let registry = mlx_gen_joycaption::provider_registry().expect("provider registry should build");
    assert!(
        registry.captioners().len() > 0,
        "provider registry must contain a captioner"
    );
