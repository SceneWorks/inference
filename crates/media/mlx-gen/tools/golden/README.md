...
| `mage_flow_vae_f32_{256,1024,2048,512x2048,768x1280}.safetensors` | `dump_mage_vae_sizes.py` | Mage-VAE **decode parity at every geometry** (sc-14039) | Same tensor names as the bf16 bundle above, so `mlx-gen-mage/tests/vae_decode_real_weights.rs` reads both through one path — but dumped in **f32**, for two reasons. (1) It is the *tighter* oracle: the bf16 bundle's residual is dominated by the reference's own rounding, while f32-vs-f32 isolates the port's error (measured: mean_abs 1.7e-4 vs bf16's 2.0e-3 at 256²). (2) torch's CPU bf16 kernels here are effectively single-threaded — a 1024² `--stage vae` run takes ~35 min and 2048 hours, while the whole f32 sweep takes ~30 s, which is what makes the DoD's explicit **2048** large-decode check practical. Keep both: the bf16 bundle measures production fidelity, the f32 bundle measures the port. The two **non-square** entries are not extras: every square geometry hides a transposed h/w, and `768x1280` (a 48x80 latent) is the only case where the CoD decoder's 32-tile attention pads **both axes by different amounts** — 1024² and 2048 pad neither. |
