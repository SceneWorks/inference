...
/// (`[curated_scheduler_names(), advanced_pass_scheduler_names()].concat()`). Selecting one builds
/// the schedule through the same [`resolve_schedule`] seam as candle, so both backends consume
...
/// The MLX half of the epic-20414 shared-schedule seam (sc-20416). Weights-free and tensor-free:
/// this drives the host-math seam only, so it runs on the plain macOS lane with no model present.
#[cfg(test)]
mod schedule_seam_tests {
    use gen_core::sampling::{
        schedule_sigmas, FlowModelSampling, ModelSampling, Scheduler, TimestepConvention,
    };
    ...
    /// `mlx_gen::resolve_schedule`; the candle twin
    /// (`candle_gen::sampler::tests::candle_seam_delivers_the_golden_schedule_coefficients`) replays
    /// the SAME table through `candle_gen::resolve_schedule`. Both passing is the statement that the
    /// two backends receive identical coefficients.
    #[test]
    fn mlx_seam_delivers_the_golden_schedule_coefficients() {
        gen_core_testkit::schedules::schedule_conformance("mlx", &|name, ms, steps| {
    ...
    /// count it returns exactly what the shared host math returns. The N1/N3 fallbacks are unchanged
    /// by the additions, including the epic's documented `bong_tangente` misspelling, which must
    /// fall back rather than resolve.
    #[test]
    fn mlx_seam_is_a_pure_delegation_and_keeps_its_native_fallbacks() {
    ...
    /// The gated advanced menu is exactly the two epic-20414 ids and stays OUT of the curated menu
    /// every family advertises — the candle twin asserts the same pair.
    #[test]
    fn advanced_pass_menu_is_gated_out_of_the_curated_menu() {
        let curated = super::curated_scheduler_names();
        let advanced = super::advanced_pass_scheduler_names();
        assert_eq!(advanced, vec!["linear_quadratic", "bong_tangent"]);
        assert_eq!(curated.len(), 8);
        for gated in &advanced {
            assert!(
                !curated.contains(gated),
