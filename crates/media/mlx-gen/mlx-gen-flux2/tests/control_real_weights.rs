...
fn control_checkpoint() -> PathBuf {
    let p = std::env::var("MLX_GEN_FLUX2_CONTROL_CHECKPOINT").unwrap_or_else(|_| panic!("set MLX_GEN_FLUX2_CONTROL_CHECKPOINT to the required snapshot dir; inference never self-fetches or derives a cache location (epic 13657)"));
    PathBuf::from(p)
}
