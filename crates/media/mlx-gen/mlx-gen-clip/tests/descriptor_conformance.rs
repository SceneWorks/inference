...
fn registered_descriptors_conform() {
    let registry = mlx_gen_clip::provider_registry().expect("provider registry should build");
    assert!(
        registry.image_embedders().len() > 0,
        "provider registry must contain an image embedder"
    );
