...
/// Spatial size grid for the **14B family**: `vae_stride_spatial (8) × patch (2) = 16` px per side
/// (vs 32 for the 5B's z48 VAE). Mirrors candle's `candle_gen_wan::config::SIZE_MULTIPLE_14B`
/// (sc-12500): consumers reject an off-grid request rather than silently align-down refitting it.
