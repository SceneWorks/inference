...
#[test]
fn flux1_variants_resolve_through_provider_catalog() {
    for id in ["flux1_schnell", "flux1_dev"] {
        let reg = mlx_gen_flux::provider_registry()
            .unwrap()
            .generators()
            .copied()
        ...
        let err = mlx_gen_flux::provider_registry()
            .unwrap()
            .load(id, &spec)
            .err()
...
#[test]
fn flux1_dev_control_resolves_through_provider_catalog() {
    // E2 (sc-8239): the finalized `flux1_dev_control` is a first-class registered model. It must
    // resolve through `provider_registry().load` like the base FLUX.1 variants. With a single
    ...
    let reg = mlx_gen_flux::provider_registry()
        ...
        .expect("provider catalog should export flux1_dev_control");
    let d = (reg.descriptor)();
    assert_eq!(d.family, "flux");
    ...
    let err = mlx_gen_flux::provider_registry()
        .unwrap()
        .load("flux1_dev_control", &spec_no_control)
        .err()
