...
//! `mx.compile` fuse the FLUX.1 MMDiT's fusable elementwise *glue* into faster kernels?
//!
//! No weights — it times the fusable chains in isolation at production shapes (inner dim 3072 =
//! 24×128, FFN 12288 = 4×dim, 19 joint + 38 single blocks, ~1024² → seq 4096), eager vs `compile`d.
//! All f32 (FLUX.1's main residual stream + RoPE run f32; the conditioning is bf16 but the affine
//! promotes to f32). The chains:
//!   * **gelu_tanh** — the tanh-GELU FFN activation on `[B, S, ffn]` (the text + single-block FFNs;
//!     the image-block exact `gelu` is already `mx.compile`'d inside mlx-rs, so it's excluded here).
...
//! Run it:
//! ```text
//! cargo test --release -p mlx-gen-flux --test integration compile_micro:: -- --ignored --nocapture
//! ```
...
fn compile_glue_micro() {
    let b = env_usize("FLUX_PERF_BATCH", 1) as i32;
    let s = env_usize("FLUX_PERF_SEQ", 4096) as i32; // 1024² image tokens
    let dim = env_usize("FLUX_DIM", 3072) as i32;
    let ffn = env_usize("FLUX_FFN", 12288) as i32; // 4 × 3072
    let heads = env_usize("FLUX_HEADS", 24) as i32;
    let half = env_usize("FLUX_HALF", 64) as i32; // head_dim/2
    let warmup = 3usize;
    let iters = 12usize;
    println!("shapes: B={b} S={s} dim={dim} ffn={ffn} heads={heads} half={half}  (warmup={warmup} iters={iters})");

    // ---- gelu_tanh FFN on [B, S, ffn] f32 (~57 / step) ----
    {
        ...
        println!(
            "[gelu_tanh f32 {b}x{s}x{ffn}] eager={eager:.3} oneshot={oneshot:.3} held={heldt:.3} ms \
             | held speedup={:.2}× saved/call={:.3}ms ×57={:.1}ms | max|Δ|={diff:.2e}",
            eager / heldt,
            eager - heldt,
            (eager - heldt) * 57.0
        );
    ...
    // ---- modulate (adaLN affine) on [B, S, dim] f32 (~115 / step) ----
    {
        ...
        println!(
            "[modulate f32 {b}x{s}x{dim}] eager={eager:.3} oneshot={oneshot:.3} held={heldt:.3} ms \
             | held speedup={:.2}× saved/call={:.3}ms ×115={:.1}ms | max|Δ|={diff:.2e}",
            eager / heldt,
            eager - heldt,
            (eager - heldt) * 115.0
        );
    ...
    // ---- gated residual on [B, S, dim] f32 (~114 / step) ----
    {
        ...
        println!(
            "[gated f32 {b}x{s}x{dim}] eager={eager:.3} oneshot={oneshot:.3} held={heldt:.3} ms \
             | held speedup={:.2}× saved/call={:.3}ms ×114={:.1}ms | max|Δ|={diff:.2e}",
            eager / heldt,
            eager - heldt,
            (eager - heldt) * 114.0
        );
    ...
    // ---- rope_rotate on [B, H, S, half] f32 (q and k, ~114 / step) ----
    {
