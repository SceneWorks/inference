//! sc-19753 — FLUX.1 **inherits** the layer-wise, normalization-correct bounded VAE decode.
//!
//! FLUX.1 owns no VAE. `loader::load_vae` / `load_vae_from_weights` build
//! [`mlx_gen_z_image::vae::Vae`] (the shared AutoencoderKL), and `model::decode_latents_via_seam`
//! decodes through `LatentDecoder::decode_tiled` on that exact type. sc-19753 converted that decoder
//! (mid-block attention hoisted into a dense head; layer-wise `GlobalGroupNorm` +
//! `tiled_conv2d_3x3_nhwc` in the tail), so FLUX.1 is covered *by inheritance* — a claim that is only
//! worth what a test proves.
//!
//! These are executed decodes, not a shape check: the weights-free z-image decoder fixture is driven
//! through the same seam body the production render takes (`unpack_latents` →
//! `ensure_decoder_compatible` → `LatentDecoder::decode_tiled`), and the bounded result must track the
//! dense decode. The compile-time bindings below fail the build if FLUX.1 ever grows its own VAE type;
...
/// **Compile-time inheritance proof.** Both production VAE loaders must return the shared
/// [`mlx_gen_z_image::vae::Vae`] *by identity*. If FLUX.1 ever grows its own decoder type — the only
/// way it could miss a fix landed in the shared one — these bindings stop compiling.
const _: fn(&Path) -> Result<Vae> = mlx_gen_flux::load_vae;
const _: fn(Weights) -> Result<Vae> = mlx_gen_flux::load_vae_from_weights;
...
/// The production seam body of `model::decode_latents_via_seam` reconstructed from public API:
/// unpack the packed DiT latent, admit the decoder for the FLUX.1 latent space, then decode —
/// bounded when a tiling config is supplied.
...
/// The inheritance proof. FLUX.1's decode seam, driven on the shared decoder, must land on its own
...
#[test]
fn flux_bounded_decode_tracks_the_dense_decode() {
    ...
    println!("[flux inheritance] bounded vs dense max|Δ|={delta:.3e}");
    assert!(
        delta < 1e-4,
        "FLUX.1's bounded decode diverged from its dense decode: max|Δ|={delta:.3e} — the shared \
         z-image decoder is no longer normalization-correct under tiling, or FLUX.1 stopped routing \
         to it"
    );
}
...
fn the_pre_fix_whole_tail_route_is_observably_wrong_against_the_dense_decode() {
    ...
    println!("[flux inheritance] pre-fix whole-tail vs dense max|Δ|={delta:.3e}");
    assert!(
        delta > 1e-1,
...
/// A pre-tripped cancel must be observed before any tensor work on the FLUX.1 seam too.
#[test]
fn flux_bounded_decode_honors_a_pretripped_cancel() {
    let (vae, latent) = fixture_vae();
