...
- **sc-3357** Wan-native first_last_frame on TI2V-5B (In Review) — `build_ti2v_multi_mask` + `build_ti2v_keyframe_z` + a `Conditioning::Keyframe` path; structural tests only (no Wan reference exists for these modes). *(**sc-19571**: `build_ti2v_multi_mask` took bare latent indices, so `Keyframe.strength` never reached the mask and both conditioning-strength sliders were inert. It is now folded into a single strength-aware `build_ti2v_mask(pins: &[(usize, f32)], …)` matching candle's signature; the symbol no longer exists.)*
