...
//!   cargo test -p mlx-gen-sd3 --release --test integration e2e_medium_real_weights:: -- --ignored --nocapture
//!
...
/// the HF hub cache.
fn snapshot() -> PathBuf {
    let p = std::env::var("SD3_MEDIUM_SNAPSHOT").unwrap_or_else(|_| panic!("set SD3_MEDIUM_SNAPSHOT to the required snapshot dir; inference never self-fetches or derives a cache location (epic 13657)"));
    PathBuf::from(p)
}
