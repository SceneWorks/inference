...
/// NaN/inf is named the same way wherever it enters: the ComfyUI Kitchen importer's per-tensor scale
/// (`Nvfp4KitchenExt::from_kitchen_parts`, candle-gen) and [`Nvfp4Tensor::pack_from_slice`]'s weight
/// elements. Public because that importer lives outside the codec (sc-24135).
///
...
pub fn non_finite_msg(what: &str, value: f32) -> String {
...
impl Nvfp4Tensor {
    ...
    /// `non_finite_msg` wording) `from_kitchen_parts` applies to its per-tensor scale. Without it the
