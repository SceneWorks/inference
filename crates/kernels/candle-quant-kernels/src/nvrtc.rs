...
//!
//! A compiled module lists its kernel entry points (`CompiledKernel::entry_points`, read from
//! the PTX), so a check that must hold for every kernel of every source (no per-thread local
//! memory, sc-24164) walks what was actually compiled rather than a hand-kept list of names.
//!
...
/// The kernel entry points (`.entry` directives) a PTX module defines, in order: the names
/// `CompiledKernel::function` (cuda builds) resolves. `extern "C"` kernels appear under their
/// source name.
pub fn ptx_entry_points(ptx: &str) -> Vec<String> {
    let mut entries: Vec<String> = Vec::new();
    for line in ptx.lines() {
        let mut tokens = line.split_whitespace();
        // Only linkage/visibility directives (`.visible`, `.weak`, ...) may precede `.entry`.
        while let Some(token) = tokens.next() {
            if token == ".entry" {
                let name = tokens
                    ...
                    .unwrap_or_default();
                if !name.is_empty() && !entries.iter().any(|entry| entry == name) {
...
mod cuda_impl {
    ...
    impl CompiledKernel {
        ...
        /// Every kernel entry point this module defines ([`ptx_entry_points`] of the PTX nvrtc
        /// produced for this device): each is a valid [`function`](Self::function) name.
    ...
    impl KernelSource {
        ...
        fn compile_uncached(&self, dev: &CudaDevice, ordinal: usize) -> Outcome {
            ...
            let entry_points = ptx_entry_points(&ptx.to_src());
...
mod tests {
    ...
    #[test]
    fn ptx_entry_points_lists_every_kernel_once_in_order() {
        let ptx = "//\n// Generated by NVIDIA NVVM Compiler\n.version 8.8\n.target sm_120\n\
                   \t// .globl\tfirst_kernel\n\
                   .visible .entry first_kernel(\n\t.param .u64 first_kernel_param_0\n)\n{\n}\n\
                   .func helper(.param .b32 x)\n{\n}\n\
                   .weak .entry second_kernel (\n)\n{\n}\n\
                   .entry _Z5thirdPf(\n)\n{\n}\n\
                   .visible .entry first_kernel(\n)\n";
        assert_eq!(
            ptx_entry_points(ptx),
            ["first_kernel", "second_kernel", "_Z5thirdPf"]
        );
        assert!(ptx_entry_points(".version 8.8\n.func only_a_device_function()\n").is_empty());
    }
...
mod cuda_tests {
    ...
    fn a_source_is_compiled_once_per_device_and_the_module_is_shared() {
        ...
        assert_eq!(first.entry_points(), ["fill"]);
