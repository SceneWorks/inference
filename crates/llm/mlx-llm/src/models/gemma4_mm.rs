...
//! packed text encoder repacks the same tensors flat (`vision_model.*`,
//! `multi_modal_projector.*`, `audio_projector.*`) with an identical `gemma_config`. The config
//! cannot tell them apart — both declare `Gemma4UnifiedForConditionalGeneration` — so
//! [`Gemma4Layout`] discriminates on a signature *tensor key* and fails closed when neither is
//! present.
...
/// The dtype the decoder computes in; the multimodal front-ends match it so their output splices
/// into the token embeddings without a cast.
...
impl Gemma4Layout {
    ...
    /// The decoder's weight-key root for this layout — what `CausalLm` prefixes its `layers.{i}.*`,
    /// `embed_tokens.weight`, and `norm.weight` lookups with.
    ...
    /// The vision embedder's tensor prefix.
    ...
    /// The vision→decoder projection's tensor prefix.
    ...
    /// The audio→decoder projection's tensor prefix.
...
///
/// Every field is read from the config rather than defaulted to a constant where the config carries
/// it: `image_token_id` and friends are checkpoint properties, and a snapshot that disagrees with
/// upstream's numbering must splice at *its* ids, not at ours.
#[derive(Clone, Debug, PartialEq)]
pub struct Gemma4MmConfig {
    /// `image_token_id` — the soft-token id each image patch row replaces (258880 upstream).
    pub image_token_id: i32,
    /// `boi_token_id` — begin-of-image, emitted before an image's soft-token run (255999 upstream).
...
pub struct Gemma4VisionConfig {
    ...
    /// `patch_size * pooling_kernel_size` = 16 * 3). Read directly when present, derived from the
    /// two factors otherwise.
    pub patch_pixels: usize,
    /// `mm_posemb_size` — the number of entries in each positional-embedding axis (1120 upstream).
    pub posemb_size: usize,
    /// `num_soft_tokens` — the maximum soft tokens one image may occupy (280 upstream).
...
pub struct Gemma4AudioConfig {
    ...
    /// The sample rate the projector's framing assumes, from the processor's feature extractor
    /// (16 000 upstream). A clip at any other rate is rejected rather than silently resampled.
    pub sample_rate: u32,
    /// `audio_seq_length` — the maximum audio soft tokens one clip may occupy (750 upstream).
    pub max_soft_tokens: usize,
}

/// Upstream's audio sample rate, used when the snapshot ships no `processor_config.json` to read it
/// from. Every shipped Gemma 4 feature extractor declares 16 kHz.
    ...
    pub fn from_json(v: &Value, processor: Option<&Value>) -> Result<Self> {
        ...
        let vision = match v.get("vision_config") {
            ...
            Some(vc) => {
                ...
                // `model_patch_size` is the authority; `patch_size * pooling_kernel_size` is the
                // documented factorization and the fallback when only the factors are present.
        ...
        let audio = match v.get("audio_config") {
            ...
            Some(ac) => {
                ...
                // `audio_samples_per_token` appears in both blocks upstream; prefer the model's own
                // `audio_config`, fall back to the feature extractor, and refuse to guess.
        ...
        Ok(Self {
            ...
            // Upstream's key really is `eoa_token_index` (the odd one out); accept `eoa_token_id`
            // too rather than failing a snapshot that spells it consistently.
            eoa_token_id: v
                .get("eoa_token_index")
...
pub struct Gemma4VisionEmbedder {
    ...
    patch_ln2_w: Array,
    patch_ln2_b: Array,
    /// `[posemb_size, hidden]` — the row axis of the `[posemb_size, 2, hidden]` table.
    pos_row: Array,
    /// `[posemb_size, hidden]` — the column axis.
    pos_col: Array,
    pos_norm_w: Array,
    pos_norm_b: Array,
    /// `multi_modal_projector` / `embed_vision`: `[hidden, hidden]`, no bias.
    proj_w: Array,
    cfg: Gemma4VisionConfig,
}

impl Gemma4VisionEmbedder {
    /// Whether `w` carries this layout's vision tensors at all (a Gemma 4 text-only re-export does
    /// not). Probes the projection that must exist for the tower to reach the decoder.
    ...
    pub fn from_weights(w: &Weights, layout: Gemma4Layout, cfg: Gemma4VisionConfig) -> Result<Self> {
        ...
        if shape[0] as usize != cfg.posemb_size {
            return Err(Error::Config(format!(
        ...
        let pos_row = pos
            ...
            .reshape(&[shape[0], hidden])?;
        let pos_col = pos
            .take_axis(Array::from_slice(&[1], &[1]), 1)?
            .reshape(&[shape[0], hidden])?;

        let patch_dense_w = get(format!("{vp}.patch_dense.weight"))?;
        let dense_shape = patch_dense_w.shape().to_vec();
        let expected_in = cfg.patch_elems() as i32;
    ...
    ///
    /// Returns `[N, hidden]`, ready to splice at the image soft-token rows.
    pub fn forward(&self, patches: &Array, grid: (usize, usize)) -> Result<Array> {
        let (gh, gw) = grid;
        let n = patches.shape()[0] as usize;
        if gh * gw != n {
            return Err(Error::Config(format!(
        ...
        let x = layer_norm(
            patches,
            Some(&self.patch_ln1_w),
            Some(&self.patch_ln1_b),
            eps,
        )?;
        let x = linear(&x, &self.patch_dense_w, Some(&self.patch_dense_b))?;
        let x = layer_norm(&x, Some(&self.patch_ln2_w), Some(&self.patch_ln2_b), eps)?;

        let (row_idx, col_idx) = posemb_indices(gh, gw, self.cfg.posemb_size);
        let rows = Array::from_slice(&row_idx, &[n as i32]);
        let cols = Array::from_slice(&col_idx, &[n as i32]);
        let pos = add(
            &self.pos_row.take_axis(&rows, 0)?,
            &self.pos_col.take_axis(&cols, 0)?,
        )?;
        let x = add(&x, &pos)?;
        let x = layer_norm(&x, Some(&self.pos_norm_w), Some(&self.pos_norm_b), eps)?;
...
impl Gemma4AudioEmbedder {
    ...
    pub fn from_weights(w: &Weights, layout: Gemma4Layout, cfg: Gemma4AudioConfig) -> Result<Self> {
        ...
        if shape.len() != 2 || shape[1] as usize != cfg.samples_per_token {
            return Err(Error::Config(format!(
                "Gemma 4 audio: `{key}` is {shape:?} but the config frames audio at \
                 {} samples per token, so the projection must take that many inputs",
    ...
    pub fn forward(&self, frames: &Array) -> Result<Array> {
        let width = frames.shape()[1] as usize;
...
impl Gemma4Mm {
    ...
    pub fn from_weights(w: &Weights, layout: Gemma4Layout, cfg: Gemma4MmConfig) -> Result<Self> {
        let vision = match (&cfg.vision, Gemma4VisionEmbedder::present(w, layout)) {
            (Some(vc), true) => Some(Gemma4VisionEmbedder::from_weights(w, layout, vc.clone())?),
            _ => None,
        };
...
// --- pure host geometry (no tensors; unit-testable without a device) ----------------------------
...
///
/// A single-position axis reads entry 0 (`floor(0 * n / 1)`), which is what the formula already
/// gives; it is called out because a 1-row or 1-column image is the degenerate case most likely to
/// be got wrong.
pub fn posemb_indices(gh: usize, gw: usize, posemb_size: usize) -> (Vec<i32>, Vec<i32>) {
    let last = posemb_size.saturating_sub(1);
    let mut rows = Vec::with_capacity(gh * gw);
    let mut cols = Vec::with_capacity(gh * gw);
    for r in 0..gh {
        // `min(last)` is belt-and-braces: the formula cannot exceed `posemb_size - 1` for r < gh,
        // but a clamp here means a future caller cannot turn a geometry bug into an OOB gather.
        let ri = ((r * posemb_size) / gh.max(1)).min(last) as i32;
        for c in 0..gw {
            let ci = ((c * posemb_size) / gw.max(1)).min(last) as i32;
            rows.push(ri);
            cols.push(ci);
        }
    }
...
/// with the most patches that still fits `max_soft_tokens`.
///
/// Both dimensions are at least 1, so even an image smaller than one patch yields a 1x1 grid rather
/// than an empty span (an image contributing zero soft tokens would splice nothing while still
/// costing its `boi`/`eoi` framing).
...
///
/// Zero-padding (rather than dropping) the tail keeps a clip shorter than one frame audible: a
/// 100-sample clip becomes one frame, not zero.
pub fn audio_frames(
    samples: &[f32],
    ...
    let frames = samples.len().div_ceil(samples_per_token).min(
        // `max(1)` so a zero cap cannot erase the clip entirely.
        max_soft_tokens.max(1),
    );
...
///
/// The chat template renders exactly one `marker` per image / audio block; the processor is what
/// expands it, which is what this reproduces. Errors when the number of markers and the number of
/// supplied counts disagree, rather than expanding some and leaving others — a silently
/// unexpanded marker would splice one feature row into a span the decoder sized differently.
...
pub fn concat_features(blocks: &[Array]) -> Result<Array> {
    match blocks {
        [] => Err(Error::Config(
...
mod tests {
    ...
    fn derives_the_patch_side_from_its_factors_when_unstated() {
        ...
        // 16 x 3 = 48, so the flattened patch is still 6912 — the factorization the config documents.
        assert_eq!(cfg.vision.unwrap().patch_pixels, 48);
    }

    #[test]
    fn a_config_missing_a_token_id_is_refused_not_defaulted() {
        // Splicing at a guessed id would put image features on ordinary text rows.
        let mut v = unified_config();
    ...
    fn audio_frames_pad_the_tail_and_cap_the_span() {
        // 700 samples at 640 per token => 2 frames, the second zero-padded.
        ...
        // Shorter than one frame still yields one frame, not zero.
        assert_eq!(audio_frames(&[0.5], 640, 750).unwrap().len(), 640);
        assert_eq!(audio_frame_count(1, 640, 750), 1);
        // The cap truncates rather than overflowing the soft-token budget.
        ...
        // An empty clip is refused, not silently framed as zeros.
        assert!(audio_frames(&[], 640, 750).is_err());
    }

    #[test]
    fn expand_framed_placeholders_wraps_each_span() {
        // One marker, 3 soft tokens: <boi> tok tok tok <eoi>, text preserved around it.
        ...
        // Two markers take their own counts, in order.
        ...
        // No markers, no counts: unchanged.
        assert_eq!(
    ...
    fn expand_framed_placeholders_refuses_a_count_mismatch() {
        // Two markers but one count: expanding the first and leaving the second would splice a
