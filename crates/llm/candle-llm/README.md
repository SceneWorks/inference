...
| `QWEN3_8B_SNAPSHOT` | the pinned `Qwen/Qwen3-8B` snapshot dir (the manifest's name for it) | `speculative_engine_parity::llama_family_qwen3_8b_exact_rows_and_ngram_knife_edge_gate` — the llama family's 256-token gate (sc-24140): static seam, fused-off loops and the CUDA-graph fallback token-identical to the reference loop; n-gram rows diverge only at enumerated bf16 knife-edges |
