...
//!
//! The goldens in `crates/llm/testdata/architectures/forward_candle.json` were generated by running
//! this exact file on `origin/feature/sc-18755-ltx-2-5` (the branch point), so a match proves this
//! branch did not move a single output value. They keep guarding after this story lands: sc-18761
...
//!
//! ```text
//! SC18769_WRITE_FORWARD_GOLDEN=1 cargo test -p candle-llm --test architecture_forward
//! ```
...
fn dense_mlp(m: &mut HashMap<String, Tensor>, p: &dyn Fn(&str) -> String, rng: &mut SplitMix64) {
...
fn qwen2_moe() -> Case {
    let (n_exp, moe_inter, shared_inter) = (4usize, 16usize, 32usize);
...
fn deepseek_v2() -> Case {
    ...
    let (n_routed, moe_inter, n_shared, dense_inter) = (4usize, 16usize, 1usize, 32usize);
...
/// (sc-18761); when it does, it belongs here.
fn cases() -> Vec<Case> {
    vec![
        llama(),
        qwen3(),
...
fn run_forward(case: &Case) -> Vec<f32> {
    ...
    for (step, tok) in DECODE_STEPS.iter().enumerate() {
        ...
        let logits = model
            .decode_logits(
                &input_ids(&[*tok], &Device::Cpu).unwrap(),
                &mut cache,
                offset,
            )
...
fn every_architecture_forward_is_bit_identical_to_the_base_branch() {
    ...
    if std::env::var("SC18769_WRITE_FORWARD_GOLDEN").is_ok() {
        ...
        doc.insert(
            "_note".into(),
            Value::String(
                "Generated by `SC18769_WRITE_FORWARD_GOLDEN=1 cargo test -p candle-llm --test \
