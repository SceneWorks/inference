//! Qwen3.6 vision encoder (`Qwen3_5VisionModel`, `model_type` `qwen3_5_vision`) — story sc-7634
//! (the candle mirror of mlx-llm sc-7698).
//!
...
//!   output (one token per merged 2×2 block) is what the decoder consumes (no separate projector; the
...
impl VisionBlock {
    fn forward(
        ...
        let y = layer_norm(x, &self.n1_w, &self.n1_b, LN_EPS)?;
        let x = x.broadcast_add(&self.attn(&y, cos, sin, num_heads, head_dim, mask)?)?;
        let y = layer_norm(&x, &self.n2_w, &self.n2_b, LN_EPS)?;
        Ok(x.broadcast_add(&self.mlp(&y)?)?)
    }
    ...
    fn attn(
        ...
        let pick = |i: usize| -> Result<Tensor> {
            Ok(qkv
                .narrow(1, i, 1)?
                .contiguous()?
                .reshape((1, n, num_heads, head_dim))?)
        };
        // RoPE expects [batch, seq, heads, head_dim]; cos/sin [1, n, head_dim] broadcast over heads.
        let q = apply_rope(&pick(0)?, cos, sin, false)?;
        let k = apply_rope(&pick(1)?, cos, sin, false)?;
        let v = pick(2)?;
        // → [1, heads, n, head_dim] for SDPA (MHA, no GQA expand needed).
...
impl Qwen35VisionModel {
    ...
    fn position_embeds(&self, grid_thw: &[[i32; 3]], n: i32) -> Result<Tensor> {
        let device = self.pos_embed.device();
        ...
        let idx_u: Vec<u32> = idx.iter().map(|&x| x as u32).collect();
        let idx_arr = Tensor::from_vec(idx_u, (4 * n as usize,), device)?;
        let gathered = self
            .pos_embed
            .index_select(&idx_arr, 0)?
            .reshape((4, n as usize, hidden))?;
        let w_arr = Tensor::from_vec(wts, (4, n as usize, 1), device)?;
        Ok(gathered.broadcast_mul(&w_arr)?.sum(0)?)
    }
}
...
fn vision_rotary(
    ...
    let shape = (1usize, n, head_dim as usize);
    Ok((
...
/// fast unmasked path).
fn block_diag_mask(cu: &[i32], n: i32, device: &Device) -> Result<Option<Tensor>> {
    if cu.len() <= 2 {
        return Ok(None);
    }
    let n = n as usize;
    ...
    Ok(Some(Tensor::from_vec(data, (1, 1, n, n), device)?))
}
...
mod tests {
    ...
    fn test_cfg() -> Qwen35VisionConfig {
        // Mirrors the oracle generator (/tmp/gen_vision.py).
        Qwen35VisionConfig {
            depth: 2,
            hidden_size: 32,
            num_heads: 4,
    ...
    /// loader (incl. the Conv3d→linear patch-weight reshape). The same framework-independent fixture as
    /// the mlx-llm port (sc-7698); candle CPU runs the patches in exact f32, so the match is tight.
    #[test]
    fn encoder_matches_qwen3_5_vision_reference() {
        ...
        let n = (grid[0][0] * grid[0][1] * grid[0][2]) as usize;
        let dev = Device::Cpu;

        let mut m: HashMap<String, Tensor> = HashMap::new();
        let mut put = |key: &str, k: &str, shape: &[usize]| {
            m.insert(
                key.to_string(),
                Tensor::from_vec(arr(&j, k), shape.to_vec(), &dev).unwrap(),
            );
        };
        let (hid, inter, out_h) = (
            cfg.hidden_size as usize,
            cfg.intermediate_size as usize,
            cfg.out_hidden_size as usize,
        );
        let merge_dim = cfg.merge_dim() as usize;
        ...
        put("model.visual.patch_embed.proj.bias", "patch_b", &[hid]);
        put(
            "model.visual.pos_embed.weight",
            "pos_embed",
            &[cfg.num_position_embeddings as usize, hid],
        );
        for i in 0..cfg.depth {
            let pre = format!("model.visual.blocks.{i}");
            put(&format!("{pre}.norm1.weight"), &format!("b{i}_n1w"), &[hid]);
            put(&format!("{pre}.norm1.bias"), &format!("b{i}_n1b"), &[hid]);
            put(&format!("{pre}.norm2.weight"), &format!("b{i}_n2w"), &[hid]);
            put(&format!("{pre}.norm2.bias"), &format!("b{i}_n2b"), &[hid]);
            put(
        ...
        put("model.visual.merger.norm.weight", "mg_norm_w", &[hid]);
        put("model.visual.merger.norm.bias", "mg_norm_b", &[hid]);
        put(
