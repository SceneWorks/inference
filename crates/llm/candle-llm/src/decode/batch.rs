//! Synchronous dynamic batching (story 7255).
//!
//! Throughput on a single CUDA stream comes from the **batch dimension**, not threads:
//! [`generate_batch`] packs N requests into one forward step so a batch of differing-length prompts
//! decodes under one device context with throughput that scales with batch occupancy — while each
//! row's output is **identical to running that request alone**.
//!
//! The host-side policy (admission, per-sequence stop/length retirement) is the backend-neutral
//! [`core_llm::schedule::Scheduler`] (landed for MLX by story 7167 and reused verbatim here); this
//! module owns only the Candle tensors that policy drives. The batch is assembled, **left-padded** to
//! a common width, prefilled together, and decoded in lockstep; finished sequences are retired and
//! the cache compacted ([`KvCache::retain_sequences`]) so the next step runs a smaller batch.
//!
...
//! sampling. (At batch > 1 the batched matmul/attention kernels reduce in a different order, so on a
//! GPU results differ at sub-ULP — enough to occasionally flip a greedy near-tie; that is numerical,
//! not a logic error.)
//!
//! ## Scope
//! This is *synchronous* dynamic batching: a batch decodes in lockstep to completion, with
//! retirement. *Iteration-level* continuous batching — admitting a new request mid-flight, which
//! needs ragged query lengths / chunked prefill — rides on the paged cache (story 7257); the
//! [`Scheduler`] already carries per-sequence offsets so that lands without reworking the policy.
...
use crate::decode::{record_lane_token, CancelFlag, LaneStep};
use crate::error::{Error, Result};
use crate::models::CausalLm;
use crate::primitives::kv_cache::KvCache;
use crate::primitives::sampler::{sample, SamplingParams, SplitMix64};

/// The pad token id stuffed into the left-pad region. Any in-vocabulary id works — the attention mask
/// blocks these positions and their outputs are never read — so `0` is a safe choice.
const PAD_ID: u32 = 0;

/// Additive-mask fill for a blocked attention position: a large finite negative (matching the
/// `attention` module's convention — avoids `-inf` propagation through the softmax kernel).
...
/// Generate for a batch of requests in one continuous-batched run, returning a [`GenerationOutput`]
/// per request (in request order). `on_event` receives `(request_index, event)` as each row streams —
/// the per-sequence analog of the single [`generate`](crate::decode::generate) callback.
///
...
pub fn generate_batch(
    ...
    while !active.is_empty() {
        ...
        let feed: Vec<u32> = active.iter().map(|l| l.next_token as u32).collect();
        let feed = Tensor::from_vec(feed, (b, 1), &device)?;
    ...
    Ok(seq_ids
        ...
        .map(|&seq| GenerationOutput {
            ...
            finish_reason: match sched.finish_reason(seq) {
                ...
                // `None` ⇒ still active when a cancel broke the loop.
...
/// Build the left-padded prefill inputs: token ids (u32), per-row RoPE positions, and the additive
/// `[n, 1, max_prompt, max_prompt]` attention mask (host f32, row-major).
fn build_prefill(requests: &[BatchRequest], max_prompt: i32) -> (Vec<u32>, Vec<i32>, Vec<f32>) {
    let n = requests.len();
    let mp = max_prompt as usize;
    ...
    for r in requests {
        ...
        for c in 0..mp {
            if c < pad {
                ...
                ids.push(r.prompt_ids[c - pad] as u32);
...
fn decode_mask(active: &[Lane], k_total: i32, dtype: DType, device: &Device) -> Result<Tensor> {
    let b = active.len();
    let mut mask = Vec::with_capacity(b * k_total as usize);
    for lane in active {
        for j in 0..k_total {
            mask.push(if j >= lane.pad_len { 0.0 } else { MASK_NEG });
        }
    }
    Ok(Tensor::from_vec(mask, (b, 1, 1, k_total as usize), device)?.to_dtype(dtype)?)
}

/// Sample the token for batch row `row` of a `[b, vocab]` logits tensor, using the lane's own sampler
/// state (identical call to the single-sequence path ⇒ identical token for identical logits).
fn sample_row(logits: &Tensor, row: usize, lane: &mut Lane) -> Result<i32> {
    let lg = logits.narrow(0, row, 1)?; // [1, vocab]
