...
//!    [`DecodeRecord::replay_fallbacks`]), so a run says which recovery it paid for.
...
mod tests {
    ...
    fn mtp_engine_is_token_identical_to_the_step_driver_for_k_1_to_5() {
        ...
        for k in 1..=5usize {
            ...
            assert_eq!(run.stats.replay_fallbacks, 0, "K={k}");
            assert_eq!(
    ...
    fn a_cache_with_per_position_rollback_never_replays_and_a_step_start_cache_replays_once() {
        ...
        assert_eq!(run_wrong.stats.replay_fallbacks, 0);
        assert_eq!(
        ...
        for k in 1..=5usize {
            ...
            assert_eq!(run_qwen.record.replay_fallbacks, 0);
        }
    }
