...
impl LlamaProvider {
    ...
    fn load_dir(dir: &Path, device: &Device, requested: Option<QuantSpec>) -> CoreResult<Self> {
        ...
        let gemma4 = if arch.is_gemma4() {
            ...
            (mm.vision.is_some() || mm.audio.is_some()).then(|| Gemma4Runtime {
                mm,
                dtype,
                device: device.clone(),
            })
    ...
    fn prepare_gemma4(
        ...
        if !clips.is_empty() {
            ...
            for clip in &clips {
                ...
                .map_err(to_core)?;
                let frames = gemma4_mm::frame_tensor(
                    &framed,
                    acfg.samples_per_token,
                    &rt.device,
                    rt.dtype,
...
pub fn can_load_vision(spec: &LoadSpec) -> bool {
    ...
    qwen_vl || gemma4_multimodal(&v, "vision_config")
}
