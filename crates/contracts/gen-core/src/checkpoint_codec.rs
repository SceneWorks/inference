                ...
                let (dtype, shape) = match tensor.residency.mode {
                    ResidencyMode::Dense => (
                        match tensor.resident_encoding {
                            ...
                            // No codec leaves a GGUF container resident as its *dense fallback*
                            // encoding (`GGUF_CONTAINER_CODEC` falls back to bf16), so this arm is
                            // reachable only from a hand-built plan; the opaque byte view is the
                            // honest projection of ggml blocks, and `data_bytes` below carries the
                            // measured size regardless.
                            WeightEncoding::GgufContainer => Dtype::U8,
                        },
                        tensor.shape.clone(),
                    ),
