...
/// Qwen-Image 2.1's 64-channel RGBA autoencoder latent (`AutoencoderKLQwenImage21`, sc-24108):
/// 16× spatial compression, unpatched (the 2.1 DiT consumes latents directly), single frame. The
/// per-channel statistics are the released `vae/config.json` `latents_mean` / `latents_std`
/// (`Qwen/Qwen-Image-2.1` @ 790c9263). A distinct space from the 2512 z16 lineage: nothing built
/// for that decoder can consume this tensor, and the identity hash keeps that fail-closed.
// One transcribed mean happens to read -0.5236 (≈ -π/6); it is a fitted statistic, not the constant.
#[allow(clippy::approx_constant)]
#[rustfmt::skip]
pub const QWEN_IMAGE_2_1_Z64_MEAN: [f32; 64] = [
...
#[rustfmt::skip]
pub const QWEN_IMAGE_2_1_Z64_STD: [f32; 64] = [
...
pub const QWEN_IMAGE_2_1_Z64_NORMALIZATION: LatentNormalizationStats =
    LatentNormalizationStats::from_vectors(
        "qwen-image-2-1-z64-v1",
        &QWEN_IMAGE_2_1_Z64_MEAN,
        &QWEN_IMAGE_2_1_Z64_STD,
    );

pub const QWEN_IMAGE_2_1_Z64_LATENT_SPACE: LatentSpace = LatentSpace {
    channels: 64,
    spatial_compression: SpatialCompression {
        height: 16,
        width: 16,
    ...
    patch_layout: LatentPatchLayout::Unpacked,
    temporal_law: LatentTemporalLaw::None,
    normalization: LatentNormalization::PerChannel(QWEN_IMAGE_2_1_Z64_NORMALIZATION),
};
