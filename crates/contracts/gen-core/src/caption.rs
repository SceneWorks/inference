...
pub struct CaptionSampling {
    ...
    /// `1.0` is a no-op (reference parity); the default `1.05` is the current shipped behavior. It is
    /// applied once per distinct id in the window (Hugging Face semantics): every distinct token among
    /// the last [`repetition_context`](Self::repetition_context) has its logit divided (positive) or
    /// multiplied (negative) by this factor exactly once, however often it occurs there.
...
impl Default for CaptionSampling {
    fn default() -> Self {
        Self {
            ...
            max_new_tokens: 256,
            seed: None,
            // Current shipped JoyCaption behavior (`1.05` over a 256-token window, once per distinct
            // id in the window). Set `repetition_penalty = 1.0` for reference-sampler parity.
