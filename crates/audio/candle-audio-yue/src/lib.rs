...
//! with `Unsupported` (landed: tokenizer sc-19376, stage 1 sc-19380, stage 2 sc-19381, codec
//! sc-19377, vocoder + splice sc-19378), so a registered provider fails closed instead of rendering
//! placeholder audio; the story named in the table replaces that module's `load`. Each module's
//! deterministic,
