...
fn resolve_source(env: &str, file: &str, nested: &str) -> WeightsSource {
    ...
    // Fall through to pinned hub resolution.
    let _ = (file, nested);
    match env {
        "MMAUDIO_VAE_SNAPSHOT" => {
            mm::output::resolve_pinned_vae().expect("resolve pinned v1-16.pth")
        }
        _ => mm::output::resolve_pinned_bigvgan().expect("resolve pinned best_netG.pt"),
    }
}
