//! Shared Qwen3-style transformer blocks (sc-13360).
//!
//! The MOSS-TTSD backbone (`config.json` → `Qwen3Model`) is a standard Qwen3 decoder stack — GQA
//! with per-head q/k RMSNorm, half-split (NeoX) RoPE at `rope_theta`, and a SiLU gated MLP. This is
//! the same verified block the sibling `candle-audio-moss-tts-realtime` crate factors out
//! (sc-13334/sc-13417), reused here weight-for-weight since MOSS-TTSD's `language_model.layers` are
//! the identical Qwen3 layer.
//!
...
//!   from scratch (the parity reference).
