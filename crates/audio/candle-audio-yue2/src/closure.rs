...
fn assemble(
    ...
    for id in components_for(decoders) {
        ...
        for file in verified.files() {
            ...
            // Two components of one repository (YuE2-3B and its `qwen.tiktoken`) may pin the same
            // file: it is copied once, and the existing copy is checked against this pin below.
