...
/// weight, so it is memory-free on a packed q4/q8 tier. Two forms:
///   * `Lora` — `scale·((x·a)·b)`, two small matmuls (sc-10640).
///   * `LokrStructured` — the Kronecker vec-trick `vec(w1·reshape(x)·w2ᵀ)` (sc-10713, the candle port of
///     mlx-gen's `Adapter::LokrStructured`); the FULL `(alpha/rank)·strength` scale is baked into
///     [`LokrFactors::w2`], so a structured LoKr applies WITHOUT ever forming the `[out,in]` delta.
///
/// Factors are held **f32** (the merge/train dtype) and cast to the activation dtype per forward (they
/// are tiny, so the cast is cheap).
enum Adapter {
...
/// forward (sc-10713 — the candle port of mlx-gen's `LokrFactors`, sc-10578 / epic 10043). `ΔW = scale·
/// kron(w1, w2)` reshapes to the base's `[out, in]`, but the Kronecker–vector identity lets us apply it
/// WITHOUT ever forming that `[out, in]` tensor: with `w1` `[a, c]` and `w2` `[b, d]` (so `out = a·b`,
/// `in = c·d`), the residual `y = x·ΔWᵀ` is
///   `Y = w1 · X · w2ᵀ`  (then flatten row-major `[.., a, b] → [.., out]`),
/// where `X = reshape(x, [.., c, d])`. Two small matmuls (`[a,c]·[..,c,d]` then `·[d,b]`) touch only the
/// factor shapes — never `[out, in]` — so a LoKr applies on a packed q4/q8 base at the same memory
/// profile as a plain LoRA. The row-major kron ordering here matches [`candle_gen::train::lora`]'s
/// `kron2d` (`out[i·b+k, j·d+l] = w1[i,j]·w2[k,l]`), so the structured residual equals the folded delta.
/// The full `(alpha/rank)·strength` scale is baked into [`w2`](Self::w2) at build time.
...
impl LokrFactors {
    /// Build the small `[a,c]`/`[b,d]` Kronecker factors from a LoKr module's factors (full `w1`/`w2` or
    /// a low-rank `w_a·w_b` product — that product is bounded by the factor dims, NEVER `out×in`), baking
    /// the FULL `scale` into `w2`. The allocation-free counterpart to
    /// [`candle_gen::train::lora::reconstruct_lokr_delta`] (which materializes the full `[out,in]` delta).
    ///
    /// **Scale differs from the fold path by design** (the two-conventions trap, sc-10578): the
    /// materialized `reconstruct_lokr_delta` bakes only `alpha/rank` and rides the user `strength` in the
    /// merge `scale`, whereas the structured residual carries no separate scale field, so the FULL
    ...
    pub fn build(
        ...
        // A tucker/CP `w2` (a 4-D conv factor, lycoris `lokr_t2`) has no 2-D matrix form — not deferrable
        // via the vec-trick. The peft LoKr format never carries it; guard anyway so a conv LoKr falls
        // back to reject/fold rather than silently mis-applying.
        if w2_t2.is_some() {
            return Ok(None);
        }
        let f32d = |t: &Tensor| t.to_dtype(DType::F32);
        // The small Kronecker factors — full, or the low-rank inner product (bounded by the factor dims,
        // NEVER `out×in`): `w1_a @ w1_b` yields the small `[a, c]`, not the packed delta.
    ...
    /// (`out = a·b`). The two matmuls touch only the small factor shapes — the full `[out, in]` delta is
    /// NEVER materialized, so this holds the same memory profile on a packed q4/q8 base as a plain LoRA.
    /// Factors are cast to the activation dtype so a bf16 stream runs bf16 (mirroring the LoRA residual).
    fn residual(&self, x: &Tensor) -> Result<Tensor> {
        let xd = x.dtype();
        ...
        // matmul against explicit `[1, …]` factors that broadcast over `N`. Mirrors the mlx-gen vec-trick
        // and sidesteps any matmul-rank ambiguity when a leading dim happens to equal `a`/`b`.
...
impl AdaptLinear {
    ...
    /// Attach a forward-time **structured LoKr** residual via the Kronecker vec-trick (sc-10713): the
    /// full `(alpha/rank)·strength` scale is already baked into `factors.w2`, so `[out,in]` is never
...
mod tests {
    ...
    // ---- Structured (deferred) LoKr — the Kronecker vec-trick (sc-10713) --------------------------
    ...
    /// **MLX/candle parity (full factors).** The structured vec-trick residual `vec(w1·X·w2ᵀ)` equals the
    ...
    fn structured_lokr_full_matches_reconstruct_delta() {
        ...
        .unwrap();
        let scale = 0.9f64;
        // Materialized reference: eff = (alpha/rank)·scale = (1/1)·0.9 = 0.9 (full scale, as the
        // structured factors bake).
        ...
        .unwrap();
        // Structured factors — no [out,in] tensor built (full scale baked into w2).
        ...
        assert!(
            ...
            "structured factors must be «  the [out,in] delta ({} vs {})",
            factors.w1.elem_count() + factors.w2.elem_count(),
            out * inp
        );
        ...
        assert!(
            dev_max < 1e-4,
            "structured LoKr residual != materialized-delta residual (max diff {dev_max})"
        );
        // Failure-capable: the residual is materially non-zero (a broken `residual()` returning zero, or
        // a dropped scale bake, would fail the parity above rather than pass vacuously).
        assert!(
            max_abs(&got) > 1e-2,
    ...
    /// **MLX/candle parity (decomposed factors).** Same identity for a low-rank LoKr (`w1_a·w1_b`,
    /// `w2_a·w2_b`): the inner products are materialized only as the SMALL `[a,c]`/`[b,d]` Kronecker
    /// factors, never `[out,in]`. `out = 3·2 = 6`, `in = 5·4 = 20`, inner rank 2.
    #[test]
    fn structured_lokr_decomposed_matches_reconstruct_delta() {
        ...
        assert!(
            dev_max < 1e-4,
            "decomposed structured LoKr != materialized delta (max diff {dev_max})"
        );
    }

    /// **Acceptance parity on a PACKED base.** The structured LoKr installs on a packed q4 base, the base
    /// stays **packed** (no `[out,in]` weight materialized), and `packed_forward + residual` reproduces
    /// `packed_forward + folded_delta` within quant tolerance: `adapted − base` cancels the (bit-identical)
    /// packed base, leaving exactly the structured residual, which equals the materialized-delta residual.
    /// Also the mutation anchor: a scale-0 LoKr is an exact no-op, and a half-scale set deviates — so a
    /// dropped/wrong scale bake breaks this test.
    #[test]
    fn structured_lokr_on_packed_matches_folded_and_stays_packed() {
        ...
        .unwrap()
        .expect("deferrable");
        // No [out,in] allocated — the factors are « the dense delta.
        ...
        let x = Tensor::randn(0f32, 1f32, (4usize, in_dim), &dev).unwrap();
        let residual_actual = (adapted.forward(&x).unwrap() - base.forward(&x).unwrap()).unwrap();
        // Reference: the materialized-delta residual x·ΔWᵀ, ΔW = scale·kron(w1,w2).
        let delta = reconstruct_lokr_delta(
            Some(&w1),
            None,
            None,
        ...
        assert!(
            dev_max < 1e-4,
            "packed structured LoKr residual != folded delta (max diff {dev_max})"
        );
        assert!(
    ...
    /// NOT deferrable via the 2-D vec-trick → `Ok(None)`, so `install_anima_residuals` rejects them on a
    /// packed tier rather than materializing. A missing `w1`/`w2` leg is a typed error, never a panic.
    #[test]
