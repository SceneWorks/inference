...
/// Load `{base}` as an [`AdaptableLinear`] at Ideogram's [`GROUP_SIZE`] — packed when `{base}.scales`
...
/// Load `{base}` as a [`TokenEmbedding`] at Ideogram's [`GROUP_SIZE`] — packed when `{base}.scales`
