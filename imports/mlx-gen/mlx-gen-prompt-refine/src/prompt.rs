...
//! text-LLM provider.
//!
//! The Rust `tokenizers` crate does not apply chat templates, so — like the JoyCaption decoder path
//! and `candle-gen-prompt-refine` — the Llama-3 instruct template is hand-assembled here as a literal
//! string. The added special-token strings (`<|start_header_id|>` etc.) map to their single token ids
//! when encoded with `add_special_tokens=false`; the `<|begin_of_text|>` BOS is prepended as an id at
//! the call site.

use mlx_gen::gen_core::TextLlmCapabilities;

/// Registered text-LLM id (the worker's `prompt_refine` job resolves this). Identical to the candle
/// backend's id so both advertise the same surface.
...
/// matching the Python `PromptRefiner`'s `DEFAULT_REFINE_MODEL` and the candle backend's default.
...
/// The prompt-refine provider's advertised capability surface (MLX backend, mac-only).
...
/// accepts a user-only turn). Byte-for-byte identical to the candle backend so both produce the same
/// prompt string.
