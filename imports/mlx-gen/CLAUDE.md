...
- `tools/` holds dev-only Python `dump_*.py`/`convert_*.py`/`build_*_tokenizer.py` scripts (golden dumps, weight converters, tokenizer builders). They run from the frozen `mflux` fork's `.venv`, never as part of the Rust build.
