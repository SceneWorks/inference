...
//!
//! Mirrors the reference `BooguImageTransformer2DModel.__init__` config surface. The Base and
//! Turbo checkpoints share the same architecture (only the DiT weights differ), so one config
//! covers both.
...
impl BooguConfig {
    ...
    pub fn from_snapshot(root: impl AsRef<Path>) -> Result<Self> {
        ...
        let text = std::fs::read_to_string(&path)
            .map_err(|e| Error::Msg(format!("boogu: read {}: {e}", path.display())))?;
        let v: serde_json::Value = serde_json::from_str(&text)
            .map_err(|e| Error::Msg(format!("boogu: parse {}: {e}", path.display())))?;
        Self::from_json(&v)
    }
    ...
    pub fn from_json(v: &serde_json::Value) -> Result<Self> {
        ...
        let cfg = Self {
            ...
            reduce_type,
            timestep_scale: f("timestep_scale", d.timestep_scale),
        };
        cfg.validate()?;
        Ok(cfg)
    }
    ...
    /// Per-patch token width fed to `x_embedder` / `ref_image_patch_embedder` (= p·p·C_in).
    pub fn patch_in_dim(&self) -> usize {
        self.patch_size * self.patch_size * self.in_channels
    }

    /// Output projection width of `norm_out` (= p·p·C_out).
    pub fn patch_out_dim(&self) -> usize {
        self.patch_size * self.patch_size * self.out_channels
    }

    /// Caption-embedder input width after the layer reduce: `mean` keeps `instruction_feat_dim`,
    /// `concat` multiplies by the number of layers.
    pub fn preprocessed_instruction_feat_dim(&self) -> usize {
        if self.reduce_type.to_lowercase().contains("cat") {
            self.num_instruction_feat_layers.max(1) * self.instruction_feat_dim
        } else {
            self.instruction_feat_dim
        }
    }

    /// Timestep-embedder / modulation width: `min(hidden_size, 1024)`.
    pub fn modulation_dim(&self) -> usize {
        self.hidden_size.min(1024)
    }

    /// SwiGLU FFN inner width: `multiple_of * ceil(inner / multiple_of)` where
    /// `inner = ffn_dim_multiplier? (4·hidden) : 4·hidden`.
    pub fn ffn_inner_dim(&self) -> usize {
        let base = 4 * self.hidden_size;
        let inner = match self.ffn_dim_multiplier {
            Some(m) => (m * base as f64) as usize,
            None => base,
        };
...
mod tests {
    ...
    fn base_config_invariants() {
        ...
        assert_eq!(c.patch_in_dim(), 64);
        assert_eq!(c.patch_out_dim(), 64);
        assert_eq!(c.preprocessed_instruction_feat_dim(), 4096);
        assert_eq!(c.modulation_dim(), 1024);
        // 4*3360 = 13440 → round up to a multiple of 256 → 13568.
        assert_eq!(c.ffn_inner_dim(), 13568);
    }
