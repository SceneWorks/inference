//! Host-side image preprocessing for the vision path (story 7262 — the Candle port of mlx-llm's
//! 7157 preprocessor).
//!
...
//! reference — reproducing PIL's *fixed-point* arithmetic, not merely "a bicubic", is what avoids the
//! ±1–2 ULP divergence at gradient cliffs. [`SiglipImageProcessor`] resizes to the model's square
//! input and applies the mean/std normalization, producing the **NCHW** tensor the Candle
//! [`SiglipVisionTower`](crate::models::SiglipVisionTower) consumes. Pure host code (no contract
//! deps); the only tensor is the final `Tensor`.
...
/// normalize per channel. Output is **NCHW** `[1, 3, size, size]` f32 (Candle's `conv2d` layout).
#[derive(Clone, Debug)]
...
impl SiglipImageProcessor {
    /// Preprocess raw interleaved RGB8 `pixels` (`width*height*3` bytes) into the SigLIP NCHW tensor
    /// on `device`.
    pub fn preprocess(
        &self,
        pixels: &[u8],
        ...
        // Resized HWC (interleaved RGB), integer-valued f32 in [0, 255].
        ...
        // Normalize and transpose HWC -> CHW so the result is row-major NCHW.
        ...
        for (i, px) in resized.chunks_exact(3).enumerate() {
            for (ch, &v) in px.iter().enumerate() {
                chw[ch * n + i] = (v / 255.0 - self.mean[ch]) / self.std[ch];
            }
        }
...
/// RGB uint8 → the Qwen3.6 vision encoder's input (story sc-7634 — the candle mirror of mlx-llm
/// sc-7699). A **dynamic-resolution** Qwen-VL-family processor (not the fixed-384 SigLIP one):
/// smart-resize each image to a patch-grid multiple, rescale + normalize, and patchify into the
/// Conv3d temporal/patch layout the [`Qwen35VisionModel`](crate::models::Qwen35VisionModel) consumes
/// — `pixel_values` `[total_patches, C·temporal·patch·patch]` already in merge-block order, plus the
/// per-image `grid_thw` (`t, h, w` in patch units).
///
/// The reference fast processor (`Qwen2VLImageProcessorFast`) resizes with torchvision BICUBIC; this
/// reuses [`resize_bicubic_u8`] (PIL BICUBIC). The two agree to ≤ 2/255 (mean ~0.004/255) — negligible
/// against the encoder's own f32-GEMM noise — so this is faithful without re-deriving torchvision's
/// kernel.
#[derive(Clone, Debug)]
pub struct Qwen35ImageProcessor {
    ...
    /// Smart-resize pixel bounds for **images** (`size.shortest_edge` / `size.longest_edge`).
    pub min_pixels: usize,
    pub max_pixels: usize,
    /// Smart-resize pixel bounds for **videos** (`video_preprocessor_config` shortest/longest edge).
    /// Distinct from the image bounds and applied over the temporal-folded pixel budget.
...
impl Default for Qwen35ImageProcessor {
    /// The Qwen3.6 `preprocessor_config.json` geometry (it reuses Qwen2VL's image processor).
    fn default() -> Self {
        Self {
            patch_size: 16,
...
impl Qwen35ImageProcessor {
    ...
    /// total pixel count within the **image** `[min_pixels, max_pixels]` and the aspect ratio as close
    /// as possible (`Qwen2VL.smart_resize`). The initial round is **half-to-even** (Python `round`),
    /// which the min/max-pixels branches then override via floor/ceil.
    ...
    pub fn smart_resize_with(
        ...
        // The pixel budget includes the temporal factor (video reference); `beta` divides out
        // `temporal` so the per-frame spatial size shrinks to fit the whole clip.
    ...
    /// on `device`. `pixel_values` is `[total_patches, C·T·P·P]` f32 in merge-block order.
    pub fn preprocess(
        &self,
        pixels: &[u8],
        width: usize,
        ...
        let n = grid_h * grid_w;
        Ok((
            Tensor::from_vec(out, (n, feat), device)?,
            vec![[1, grid_h as i32, grid_w as i32]],
        ))
    }

    /// Preprocess a sampled **video** — a list of already-decoded RGB8 frames (each `width*height*3`
    /// bytes, all the same size) — into the `(pixel_values_videos, video_grid_thw)` the
    /// [`crate::models::Qwen35VisionModel`] consumes on `device`, mirroring
...
mod tests {
    ...
    fn preprocess_video_grid_and_temporal_layout() {
        ...
        let (pixels, grid) = proc.preprocess_video(&frames, &Device::Cpu).unwrap();
        ...
        assert_eq!(
            pixels.dims(),
            &[(grid[0] * grid[1] * grid[2]) as usize, feat]
        );

        // Inner feature layout per row is (channel, temporal, patch_row, patch_col). The first
        // `P*P` values are channel-0 / temporal-0 (frame 0); the next `P*P` are channel-0 /
        // temporal-1 (frame 1). Frame 0's red = (10/255 - 0.5)/0.5; frame 1's red differs — proving
        // the two distinct frames occupy the temporal axis (an image would replicate one frame here).
    ...
    fn preprocess_video_rejects_mismatched_frames() {
        ...
        assert!(proc.preprocess_video(&frames, &Device::Cpu).is_err());
        assert!(proc.preprocess_video(&[], &Device::Cpu).is_err());
    }
    ...
    /// pure rescale+normalize+reshape. A tight tolerance pins the merge-block patch order, the
    /// (C,T,P,P) inner layout, and the temporal replication.
    #[test]
    fn patchify_layout_matches_reference() {
        ...
        let (pv, grid) = proc.preprocess(&img, w, h, &Device::Cpu).unwrap();
    ...
    fn preprocess_normalizes_to_minus_one_one() {
        // 384x384 so no resize; mean/std 0.5 maps 0->-1, 255->1, 128->~0.004. NCHW: channel-major.
        let pixels = [0u8, 128, 255].repeat(384 * 384);
        let out = SiglipImageProcessor::default()
            .preprocess(&pixels, 384, 384, &Device::Cpu)
            .unwrap();
        assert_eq!(out.dims(), &[1, 3, 384, 384]);
        let v = host_f32(&out);
        let n = 384 * 384;
        // All of channel 0 came from the 0-valued R bytes -> -1; channel 2 from 255 -> 1.
        assert_eq!(v[0], -1.0);
        assert!((v[n] - 0.003_921_628).abs() < 1e-6); // channel 1 (128)
        assert_eq!(v[2 * n], 1.0); // channel 2 (255)
    }

    #[test]
    fn preprocess_rejects_bad_buffer() {
        assert!(SiglipImageProcessor::default()
            .preprocess(&[0u8; 3], 2, 2, &Device::Cpu)
            .is_err());
    }
    ...
    fn resize_uniform_image_is_uniform() {
        ...
        let src = [200u8, 100, 50].repeat(64 * 48); // width=64, height=48
