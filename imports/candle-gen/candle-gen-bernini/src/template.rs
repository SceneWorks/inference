//! The Bernini planner's ChatML templating (`BerniniTemplate.encode_messages`). Candle sibling of
//! `mlx-gen-bernini/src/template.rs` (sc-5136).
//!
...
//! `assistant` message (`has_loss 1`: the gen-target visual). The special-token markers carry no
//! tokens.
//!
...
//! `image_pad`/`video_pad` ids; this port emits the **plain pad ids directly** and tracks `token_type`
//! (`2` input-vit / `3` gen-output), `token_segment_ids` (`visual_id + 1` on a visual's pads, else the
//! position index), and `flex_token_types` (the gen visual's running indicator id) **during
//! assembly** — equivalent because special tokens always split BPE (proven by the golden).
//!
...
/// The structural outputs of [`BerniniTemplate::encode_messages`] (all host-side `Vec`s; the tensors
/// — `position_ids`, the 4-D mask — are built by [`crate::process`] from these).
...
impl BerniniTemplate {
    ...
    pub fn from_tokenizer_file(path: impl AsRef<std::path::Path>) -> CResult<Self> {
        let tok = Tokenizer::from_file(path.as_ref())
            .map_err(|e| CandleError::Msg(format!("load tokenizer: {e}")))?;
        Ok(Self { tok })
    }

    fn encode(&self, text: &str) -> CResult<Vec<i64>> {
        let enc = self
            .tok
            .encode(text, false)
    ...
    pub fn encode_messages(
        ...
        for m in conversation {
            ...
            match ty {
                ...
                other => return Err(CandleError::Msg(format!("unknown message type: {other}"))),
            }
        }
        if started {
    ...
    fn emit_message(&self, role: &str, ops: &[Op], out: &mut TemplateOutput) -> CResult<()> {
    ...
    /// Tokenize and append any buffered text (a plain text token run), then clear the buffer.
    fn flush(&self, buf: &mut String, out: &mut TemplateOutput) -> CResult<()> {
        if !buf.is_empty() {
