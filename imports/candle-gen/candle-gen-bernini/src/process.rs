//! Bernini planner **host-side data / templating** — task + media → the planner's tokenized inputs
//! (token slots, 3-D MRoPE position ids, 4-D flex attention mask). The candle sibling of
//! `mlx-gen-bernini/src/process.rs` (sc-5136); this is pure host computation (no model graph), so it
//! ports 1:1 and is validated bit-exact against the same `process_golden.safetensors` the MLX lane
//! asserts (see `tests/process_parity.rs`).
//!
//!   - [`generate_unified_inputs`] — task + media → the `inputs_structure` conversation.
//!   - [`mrope_position_ids`] — `Qwen2_5_VLModel.get_rope_index`: the 3-D MRoPE `(3, L)` ids.
//!   - [`build_attention_mask_4d`] — `build_custom_attention_mask`: the `(1, L, L)` additive flex mask.
...
///
/// `input_image_hw` is `(height, width)` per input image; `input_video_count` is the number of input
/// videos (each a bare `video` slot). `output_t == 1` selects image generation (`[SOG]` / `image_gen` /
/// `[EOG]`), otherwise video generation (`[SOV]` / `video_gen` / `[EOV]`). Mirrors the reference index
...
/// mask). `image_grid_thw` / `video_grid_thw` are the per-item `[t, h, w]` grids in the order their pad
/// runs appear in `input_ids`. With no vision items this is the pure-text ramp `[0..L)` on all three
/// rows. Returns an `[3, L]` I64 tensor (the ids are integer-exact vs the reference).
pub fn mrope_position_ids(
    ...
    if image_grid_thw.is_empty() && video_grid_thw.is_empty() {
        ...
        for _ in 0..(image_nums + video_nums) {
            ...
            // silently corrupting the MRoPE positions. Reject the malformed token sequence instead.
            if ed > l {
    ...
    Ok(Tensor::from_vec(data, (3, l), &Device::Cpu)?)
}
...
pub fn build_attention_mask_4d(token_type: &[i32], token_segment_ids: &[i32]) -> CResult<Tensor> {
    ...
    Ok(Tensor::from_vec(data, (1, l, l), &Device::Cpu)?)
}
...
mod tests {
    ...
    fn mrope_text_only_ramp() {
        ...
        assert_eq!(pos.dims(), &[3, 4]);
        let v: Vec<i64> = pos.flatten_all().unwrap().to_vec1::<i64>().unwrap();
    ...
    fn mask_gen_bidirectional_within_segment() {
        ...
        let v: Vec<f32> = m.flatten_all().unwrap().to_vec1::<f32>().unwrap();
