...
//!    prefix strip.)
//!
//! The Qwen-Image **VAE** and **text encoder** are NOT read from the ComfyUI tree in this slice: the
//! tree's VAE (`vae/qwen_image_vae.safetensors`) uses native WAN-VAE key names (a full 194-key 3D-VAE
//! remap, a separate follow-up) and the tree's Qwen2.5-VL text encoders are themselves *scaled*-fp8
//! (sc-10671 territory). Both are standard, shared components we already ship — so the ComfyUI DiT is
//! read in place while the text encoder / VAE / tokenizer come from a resident Qwen-Image diffusers
//! snapshot (the same shape [`crate::vae::QwenVae`]/[`crate::text_encoder::QwenTextEncoder`] already
//! load). This mirrors how the Z-Image lane sourced its tokenizer from our snapshot, and how the Krea 2
//! ConvRot lane swaps only the DiT while reusing a snapshot's TE/VAE/tokenizer.
//!
