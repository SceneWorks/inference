//! Z-Image packed-load seam (sc-9408, sc-9089 umbrella) — the candle twin of the flux2-dev
//! conversion (sc-9087) built on the shared [`candle_gen::quant`] packed-load module (sc-9086).
//!
//! Z-Image ships a **pre-quantized** MLX tier (`SceneWorks/z-image-turbo-mlx`, epic 8506) whose q4/q8
//! snapshots store each quantized `Linear` / token embedding as the MLX packed triple `{base}.weight`
//! (u32 codes) + `{base}.scales` + `{base}.biases`. [`QLinear::linear_detect`] / [`QEmbedding::detect`]
//! packed-**detect** the `.scales` sibling and build the quantized weight **straight from the packed
//! parts** on the target device via the shared [`candle_gen::quant::lin`] / `embedding` loaders (Q4 →
//! `Q4_1` lossless repack, Q8 → `Q8_0` requant). **No dense bf16 weight is ever materialized** on the
//! packed path — the q4 DiT lands ~3.4 GB directly rather than staging a ~28 GB dense DiT in RAM first.
//!
//! Absent `.scales` (a dense bf16 tier — the stock `Tongyi-MAI/Z-Image-Turbo` snapshot) the **dense**
//! path is taken **unchanged**, so one vendored model serves both a dense bf16 and a packed snapshot.
...
/// The Qwen3 TE `model.embed_tokens` is packed in the q4/q8 tiers, so this closes the packed-detect
...
impl QEmbedding {
    /// **Packed-detecting** `[vocab, hidden]` embedding loader (sc-9408): packed when `{base}.scales`
...
/// `VarBuilder::from_tensors` overlay. This still loads *from the packed parts* (no dense bf16 tier is
/// downloaded); the ~2 MB one-time dequant is trivial next to the DiT/TE the packed path serves natively.
pub fn dequant_packed_to_dense(
    ...
    let grid = match shared::mlx_packed_bits(wq_cols, s_cols) {
        ...
        b => candle_gen::candle_core::bail!("z-image vae: unsupported MLX packed bit-width {b}"),
    };
...
mod tests {
    ...
    /// **Packed-detect fires on the z-image key layout (incl. the `attention.to_out.0` nesting).**
    ...
    fn linear_detect_fires_on_to_out_remap_and_leaves_dense_unchanged() -> Result<()> {
        ...
        // The `attention.to_out.0` packed triple (the nested key the z-image DiT threads as one base).
        map.insert("attention.to_out.0.weight".into(), wq);
        map.insert("attention.to_out.0.scales".into(), s);
        map.insert("attention.to_out.0.biases".into(), b);
        // A dense sibling (`to_q`) with no `.scales` → the dense path must stay unchanged.
        map.insert(
            "attention.to_q.weight".into(),
    ...
    /// rows (the Qwen3 TE `model.embed_tokens` is packed in the q4/q8 tiers), while a `.scales`-less
    /// table stays dense.
    #[test]
    ...
    /// The VAE dequant-to-dense helper reproduces the affine grid a packed VAE attention weight
    ...
    fn dequant_packed_to_dense_matches_grid() -> Result<()> {
        ...
        let dense = dequant_packed_to_dense(&vb, "to_q", &dev, DType::F32)?;
        assert_eq!(dense.dims(), &[out_dim, in_dim]);
        let grid_t = Tensor::from_vec(grid, (out_dim, in_dim), &dev)?;
