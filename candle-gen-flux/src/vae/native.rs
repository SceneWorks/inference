...
impl candle_gen::candle_core::Module for AttnBlock {
...
impl ResnetBlock {
    fn new(in_c: usize, out_c: usize, vb: VarBuilder) -> Result<Self> {
        let conv_cfg = candle_gen::candle_nn::Conv2dConfig {
            padding: 1,
            ..Default::default()
        };
...
impl candle_gen::candle_core::Module for ResnetBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let h = xs
            .apply(&self.norm1)?
...
impl candle_gen::candle_core::Module for Downsample {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl candle_gen::candle_core::Module for Upsample {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl candle_gen::candle_nn::Module for Encoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl candle_gen::candle_nn::Module for Decoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl candle_gen::candle_nn::Module for DiagonalGaussian {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl candle_gen::candle_core::Module for AutoEncoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
#[cfg(test)]
mod parity_tests {
    //! Pin the vendored BFL/native FLUX AutoEncoder to the stock candle-transformers module
    //! (sc-11154 / F-081): built from the *same* `VarMap`-backed weights, the `decode` forward must be
    //! bit-identical. `decode` is deterministic (the random `DiagonalGaussian` sampling is encode-only),
    //! and the budgeted mid-block attention is a single un-chunked pass below `ATTN_SCORES_BUDGET`, so
    //! this proves the only edited line (`scaled_dot_product_attention`) changed nothing numerically.
    use super::{AutoEncoder, Config};
    use candle_gen::candle_core::{DType, Device, Tensor};
    use candle_gen::candle_nn::{VarBuilder, VarMap};
    use candle_transformers::models::flux::autoencoder as stock;

    #[test]
    fn vendored_flux_ae_decode_matches_stock() {
        let dev = Device::Cpu;
        let vm = VarMap::new();
        let vb = VarBuilder::from_varmap(&vm, DType::F32, &dev);
        // Vendored builds first, populating the VarMap; the stock module then reads the SAME
        // (identical name/shape) parameters, so any output difference is a forward-logic difference.
        let cfg = Config::dev();
        let vendored = AutoEncoder::new(&cfg, vb.clone()).unwrap();
        let stock_ae = stock::AutoEncoder::new(&stock::Config::dev(), vb).unwrap();

        let z = Tensor::randn(0f32, 1f32, (1, cfg.z_channels, 8, 8), &dev).unwrap();
        let y_v = vendored.decode(&z).unwrap();
        let y_s = stock_ae.decode(&z).unwrap();
        ...
        assert!(
            diff < 1e-5,
            "vendored FLUX AE decode diverged from stock by {diff}"
        );
    }
}
