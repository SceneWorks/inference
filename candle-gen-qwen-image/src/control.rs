...
impl QwenControl {
    ...
    pub fn load(paths: &QwenControlPaths) -> Result<Self> {
        ...
        // The base MMDiT packed-detects (a packed MLX base tier loads straight from the packed parts;
        // a dense base snapshot unchanged) at the `group_size` read from `transformer/config.json`.
