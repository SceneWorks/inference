//! SANA-1.6B txt2img GPU-validation harness (sc-11780) — resolves THIS crate's inventory-registered
//! generator through `gen_core::registry::load("sana_1600m", …)`, runs a real `generate` against an
...
fn main() -> Result<()> {
    ...
    let gen = gen_core::registry::load("sana_1600m", &spec)?;
    println!(
